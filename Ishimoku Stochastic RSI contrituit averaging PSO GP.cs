...
if (_rsiPeriod != null && _rsiPeriod.ValueInt > 0)
{
    ...
    public class TrailingStopComponent : ITradingComponent
    {
        ...
        public void ActivateTrailing(int positionId, bool isLong, decimal currentPrice)
        {
            ...
            // При активации используем currentPrice как basePrice (экстремальная цена еще не установлена)
            decimal initialTrailingLevel = CalculateTrailingLevel(positionId, currentPrice, currentPrice, isLong);
        ...
        public bool CheckTrailingStop(int positionId, decimal currentPrice, Position position)
        {
            try
            {
                ...
                {
                    // ✅ ЭТАЛОН: Рассчитываем уровень трейлинга от currentPrice
                    decimal trailingLevel = CalculateTrailingLevel(positionId, currentPrice, currentPrice, isLong);
        ...
        private decimal CalculateSelfLearningTrailingLevel(int positionId, decimal basePrice, decimal currentPrice, bool isLong)
        {
            try
            {
                ...
                // ✅ ИСПРАВЛЕНО: Используем currentPrice для расчета прибыли (для адаптации дистанции)
                // Для адаптации дистанции нужна текущая прибыль, а не от экстремума
                // basePrice используется только для расчета уровня трейлинга (ниже)
                decimal currentProfitPercent = isLong 
                    ? (currentPrice - entryPrice) / entryPrice * 100m
                    : (entryPrice - currentPrice) / entryPrice * 100m;
        ...
        private async Task CheckTradingSignals(Candle candle)
        {
            await Task.Run(() =>
            {
                try
                {
                    ...
                    bool ichimokuReady = tenkanValue != 0 && kijunValue != 0;

                    // Стохастик — ДОЛЖЕН работать независимо от того, готов ли Ишимоку
                    ...
                    // ✅ COUNTERINTUITIVE — работает независимо от того, готов ли Ишимоку
                    bool counterintuitiveReady = dataComponent.TryGetCounterintuitiveEmaValues(
                        out decimal ema1, out decimal ema2, out decimal ema3);
                    ...
                    bool openByRsi = _cachedOpenByRsi ?? IsParameterOn(_openByRsi);
                    ...
                    // ✅ RSI сигналы: выход из зон перепроданности/перекупленности
                    // LONG вход: RSI выходит из зоны перепроданности снизу вверх (было < oversold, стало >= oversold)
                    bool rsiLongSignal = rsiReady && openByRsi &&
                        previousRsi < _rsiOversold.ValueDecimal && currentRsi >= _rsiOversold.ValueDecimal;
                    // SHORT вход: RSI выходит из зоны перекупленности сверху вниз (было > overbought, стало <= overbought)
                    bool rsiShortSignal = rsiReady && openByRsi &&
                        previousRsi > _rsiOverbought.ValueDecimal && currentRsi <= _rsiOverbought.ValueDecimal;
                    
                    // ✅ COUNTERINTUITIVE сигналы: вход на откате при тренде
                    // LONG вход: тренд вверх (ema2 > ema1) и цена в откате ниже быстрой и контртрендовой EMA
                    bool counterintuitiveLongSignal = counterintuitiveReady && counterintuitiveEntry && useCounterintuitive &&
                        ema2 > ema1 && candle.Close < ema2 && candle.Close < ema3;
                    // SHORT вход: тренд вниз (ema2 < ema1) и цена в откате выше быстрой и контртрендовой EMA
                    bool counterintuitiveShortSignal = counterintuitiveReady && counterintuitiveEntry && useCounterintuitive &&
                        ema2 < ema1 && candle.Close > ema2 && candle.Close > ema3;

                    // Ишимоку-сигналы считаем ТОЛЬКО если сам Ишимоку готов
                    bool ichimokuBuySignal = ichimokuReady &&
                        CheckBuySignals(tenkanValue, kijunValue, candle.Close,
                    ...
                    bool ichimokuSellSignal = ichimokuReady &&
                        CheckSellSignals(tenkanValue, kijunValue, candle.Close);

                    // Сигналы Ишимоку: учитываются отдельно от стохастика
                    bool buySignalActive  = ichimokuBuySignal;
                    bool sellSignalActive = shortTradingEnabled && ichimokuSellSignal;
                    
                    // ================================
                    // 1) ВХОДЫ ПО СТОХАСТИКУ — ПОЛНОСТЬЮ НЕЗАВИСИМЫЕ ОТ ИШИМОКУ, RSI И НАЛИЧИЯ ПОЗИЦИЙ
                    // ================================
                    if (stochLongSignal)
                    {
                        _pendingOpenReason =
                            $"Stochastic: K {currentK:F2} > D {currentD:F2} в зоне перепроданности ({_stochOversold.ValueDecimal:F2})";
                        
                        _stateMachine?.ProcessEvent(TradingEvent.BuySignalDetected, candle);
                        TryOpenLongPosition(candle); // дальше все лимиты/защиты проверяются внутри
                    }

                    if (stochShortSignal && shortTradingEnabled)
                    {
                        _pendingOpenReason =
                            $"Stochastic: K {currentK:F2} < D {currentD:F2} в зоне перекупленности ({_stochOverbought.ValueDecimal:F2})";
                    ...
                    // ================================
                    // 2) ВХОДЫ ПО RSI — ПОЛНОСТЬЮ НЕЗАВИСИМЫЕ ОТ ИШИМОКУ, СТОХАСТИКА И НАЛИЧИЯ ПОЗИЦИЙ
                    // ================================
                    if (rsiLongSignal)
                    {
                        _pendingOpenReason =
                            $"RSI: выход из зоны перепроданности ({previousRsi:F2} → {currentRsi:F2}, порог {_rsiOversold.ValueDecimal:F2})";
                        
                        _stateMachine?.ProcessEvent(TradingEvent.BuySignalDetected, candle);
                        TryOpenLongPosition(candle);
                    }

                    if (rsiShortSignal && shortTradingEnabled)
                    {
                        _pendingOpenReason =
                            $"RSI: выход из зоны перекупленности ({previousRsi:F2} → {currentRsi:F2}, порог {_rsiOverbought.ValueDecimal:F2})";
                    ...
                    // ================================
                    // 3) ВХОДЫ ПО COUNTERINTUITIVE — ПОЛНОСТЬЮ НЕЗАВИСИМЫЕ ОТ ИШИМОКУ, СТОХАСТИКА, RSI И НАЛИЧИЯ ПОЗИЦИЙ
                    // ================================
                    if (counterintuitiveLongSignal)
                    {
                        _pendingOpenReason =
                            $"Counterintuitive: EMA2({ema2:F4}) > EMA1({ema1:F4}) [тренд вверх], цена({candle.Close:F4}) < EMA2 и < EMA3 [откат]";
                        
                        _stateMachine?.ProcessEvent(TradingEvent.BuySignalDetected, candle);
                        TryOpenLongPosition(candle);
                    }

                    if (counterintuitiveShortSignal && shortTradingEnabled)
                    {
                        _pendingOpenReason =
                            $"Counterintuitive: EMA2({ema2:F4}) < EMA1({ema1:F4}) [тренд вниз], цена({candle.Close:F4}) > EMA2 и > EMA3 [откат]";
                    ...
                    // ================================
                    // 4) ВХОДЫ ПО ИШИМОКУ — ПОЛНОСТЬЮ НЕЗАВИСИМЫЕ ОТ СТОХАСТИКА, RSI, COUNTERINTUITIVE И НАЛИЧИЯ ПОЗИЦИЙ
                    // ================================
                    // Сигналы для LONG по Ишимоку
                    if (buySignalActive)
                    {
                    ...
                    // Сигналы для SHORT по Ишимоку (если разрешено)
                    if (sellSignalActive)
                    {
                        ...
                        _stateMachine?.ProcessEvent(TradingEvent.SellSignalDetected, candle);
                        TryOpenShortPosition(candle); // дальше все лимиты/защиты проверяются внутри
                    }
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    foreach (var position in activePositions)
                    {
                        ...
                        // ✅ ПРИОРИТЕТ 4: СИГНАЛЫ ИНДИКАТОРОВ
                        // ✅ КРИТИЧНО: Защита от убытков при выходе по сигналам индикаторов
                        ...
                        // ✅ КРИТИЧНО: Защита от убытков при выходе по counterintuitive
                        ...
                        // ✅ ИСПОЛЬЗУЕМ КЭШИРОВАННЫЙ ПАРАМЕТР
                        // ✅ КРИТИЧНО: Защита от убытков при выходе по стохастику
                        ...
                        // ✅ ИСПОЛЬЗУЕМ КЭШИРОВАННЫЙ ПАРАМЕТР
                        // ✅ КРИТИЧНО: Защита от убытков при выходе по RSI
        ...
        private void TryOpenLongPosition(Candle candle)
        {
            try
            {
                ...
                _context.SendLog(
                    ...
                    $"Объем: {_volume.ValueInt} | Сигнал: {longReason}",
                    LogMessageType.Trade);
            }
            catch (Exception ex)
            {
        ...
        private bool CheckManualTakeProfit(Position position, Candle candle)
        {
            ...
            try
            {
                ...
                if (wasPositive)
                {
                    ...
                    {
                        decimal takeProfitPercent = _manualTakeProfit?.ValueDecimal ?? 0m;
                        ...
                        if (profitPercent < takeProfitPercent)
                        {
                            _context.SendLog(
                                ...
                                $"(минимальная прибыль: {minProfitPercent:F2}%, тейк-профит: {takeProfitPercent:F2}%). " +
                ...
                bool takeProfitCondition = priceReachedTakeProfit;

                // ✅ ДОПОЛНИТЕЛЬНАЯ ЗАЩИТА: Абсолютная защита от убытков по ТЗ п.4.5
                // Проверяем возможность закрытия с учетом защиты от ухода из плюса в минус
                bool lossProtectionAllowsClose = riskManager.CanClosePosition(positionId, currentPrice,
                    position.Direction == Side.Buy);

                // ✅ Тейк-профит срабатывает ТОЛЬКО если достигнут уровень И разрешено защитой
                return takeProfitCondition && lossProtectionAllowsClose;
            }
            catch
            {
                return false;
