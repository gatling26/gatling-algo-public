...
namespace OsEngine.Robots
{
    ...
    internal static class SharedDataKeys
    {
        ...
        public const string RsiOversold = "RsiOversold";
        public const string RsiOverbought = "RsiOverbought";
        ...
        public const string OpenByRsi = "OpenByRsi";
        ...
        public const string ExitByRsi = "ExitByRsi";
        ...
        // Самообучаемый следящий стоп
        public const string SelfLearningBaseDistance = "SelfLearningBaseDistance";
        public const string SelfLearningHighProfitMultiplier = "SelfLearningHighProfitMultiplier";
        public const string SelfLearningLowProfitMultiplier = "SelfLearningLowProfitMultiplier";
        public const string SelfLearningHighVolatilityMultiplier = "SelfLearningHighVolatilityMultiplier";
        public const string SelfLearningLowVolatilityMultiplier = "SelfLearningLowVolatilityMultiplier";
        public const string SelfLearningMinDistance = "SelfLearningMinDistance";
        public const string SelfLearningMaxDistance = "SelfLearningMaxDistance";
        public const string SelfLearningHistorySize = "SelfLearningHistorySize";
        public const string SelfLearningProfitThresholdHigh = "SelfLearningProfitThresholdHigh";
        public const string SelfLearningProfitThresholdLow = "SelfLearningProfitThresholdLow";
        public const string SelfLearningVolatilityThresholdHigh = "SelfLearningVolatilityThresholdHigh";
        public const string SelfLearningVolatilityThresholdLow = "SelfLearningVolatilityThresholdLow";
        ...
        // Флаги оптимизации RSI
        public const string OptimizeRsiPeriod = "OptimizeRsiPeriod";
        public const string OptimizeRsiOversold = "OptimizeRsiOversold";
        public const string OptimizeRsiOverbought = "OptimizeRsiOverbought";
        ...
        // ✅ Флаги оптимизации параметров самообучаемого трейлинга
        public const string OptimizeSelfLearningBaseDistance = "OptimizeSelfLearningBaseDistance";
        public const string OptimizeSelfLearningHighProfitMultiplier = "OptimizeSelfLearningHighProfitMultiplier";
        public const string OptimizeSelfLearningLowProfitMultiplier = "OptimizeSelfLearningLowProfitMultiplier";
        public const string OptimizeSelfLearningHighVolatilityMultiplier = "OptimizeSelfLearningHighVolatilityMultiplier";
        public const string OptimizeSelfLearningLowVolatilityMultiplier = "OptimizeSelfLearningLowVolatilityMultiplier";
        public const string OptimizeSelfLearningMinDistance = "OptimizeSelfLearningMinDistance";
        public const string OptimizeSelfLearningMaxDistance = "OptimizeSelfLearningMaxDistance";
        public const string OptimizeSelfLearningHistorySize = "OptimizeSelfLearningHistorySize";
        public const string OptimizeSelfLearningProfitThresholdHigh = "OptimizeSelfLearningProfitThresholdHigh";
        public const string OptimizeSelfLearningProfitThresholdLow = "OptimizeSelfLearningProfitThresholdLow";
        public const string OptimizeSelfLearningVolatilityThresholdHigh = "OptimizeSelfLearningVolatilityThresholdHigh";
        public const string OptimizeSelfLearningVolatilityThresholdLow = "OptimizeSelfLearningVolatilityThresholdLow";
    ...
    internal static class ParameterGroups
    {
        ...
        public const string TrailingStop = "Следящий стоп";
    }
    ...
    public class AdaptiveTradingStateMachine : ITradingComponent, IStateMachine
    {
        ...
        // Условные методы
        private bool IsBlocked(object data)
        {
            try
            {
                // Проверяем готовность основных индикаторов
                var dataComponent = _context.GetComponent<DataIndicatorComponent>();
                if (dataComponent == null) return true;

                // Проверяем готовность Ишимоку (основной индикатор)
                decimal tenkan = dataComponent.GetTenkanValue();
                decimal kijun = dataComponent.GetKijunValue();

                bool ichimokuReady = tenkan != 0 && kijun != 0;

                if (!ichimokuReady)
                {
                    // Логируем только при первой проверке или редко, чтобы не засорять лог
                    if (_context.SharedData.TryGetValue("LastIchimokuBlockLog", out var lastLogObj) &&
                        lastLogObj is DateTime lastLog)
                    {
                        if (DateTime.Now - lastLog > TimeSpan.FromMinutes(1))
                        {
                            _context.SendLog("🚫 Переход в режим мониторинга заблокирован: индикатор Ишимоку не готов (недостаточно исторических данных)", LogMessageType.System);
                            _context.SharedData["LastIchimokuBlockLog"] = DateTime.Now;
                        }
                    }
                    else
                    {
            ...
            {
                return true; // В случае ошибки блокируем переход
            }
        }
    ...
    public class DataIndicatorComponent : ITradingComponent
    {
        ...
        private StrategyParameterDecimal _rsiOversold;

        private StrategyParameterDecimal _rsiOverbought;
...
// === RSI ===
if (context.SharedData.TryGetValue(SharedDataKeys.RsiPeriod, out var rsiPeriod))
    _rsiPeriod = rsiPeriod as StrategyParameterInt;
if (context.SharedData.TryGetValue(SharedDataKeys.RsiOversold, out var rsiOs))
    _rsiOversold = rsiOs as StrategyParameterDecimal;
if (context.SharedData.TryGetValue(SharedDataKeys.RsiOverbought, out var rsiOb))
    _rsiOverbought = rsiOb as StrategyParameterDecimal;
...
CreateIndicators();
        ...
        private void CreateIndicators()
        {
            try
            {
                ...
                if (_rsi != null)
                {
                    try { _tab.DeleteCandleIndicator(_rsi); } catch { }
                    _rsi = null;
                }
                ...
                // === RSI ===
if (_context.SharedData.TryGetValue(SharedDataKeys.RsiPeriod, out var rsiPeriod))
{
    _rsiPeriod = rsiPeriod as StrategyParameterInt;
}
if (_context.SharedData.TryGetValue(SharedDataKeys.RsiOversold, out var rsiOs))
{
    _rsiOversold = rsiOs as StrategyParameterDecimal;
}
if (_context.SharedData.TryGetValue(SharedDataKeys.RsiOverbought, out var rsiOb))
{
    _rsiOverbought = rsiOb as StrategyParameterDecimal;
}

// ✅ СОЗДАНИЕ ИНДИКАТОРА RSI
if (_rsiPeriod != null && _rsiPeriod.ValueInt > 0)
{
    try
    {
        _rsi = IndicatorsFactory.CreateIndicatorByName("Rsi",
            "Rsi_" + _rsiPeriod.ValueInt, false);
        if (_rsi.ParametersDigit != null && _rsi.ParametersDigit.Count >= 1)
        {
            _rsi.ParametersDigit[0].Value = _rsiPeriod.ValueInt;
        }
        _rsi.Save();
        // ✅ Добавляем RSI на график в области NewArea0 (вместе со Stochastic)
        _rsi = (Aindicator)_tab.CreateCandleIndicator(_rsi, "NewArea0");
        if (_rsi != null)
        {
            _context.SendLog($"✅ RSI успешно добавлен на график в области NewArea0", LogMessageType.System);
        }
        else
        {
            _context.SendLog($"⚠️ Ошибка: RSI не был добавлен на график (вернулся null)", LogMessageType.Error);
        }
    }
    catch (Exception ex)
    {
        _context.SendLog($"⚠️ Ошибка создания индикатора RSI: {ex.Message}", LogMessageType.Error);
        _rsi = null;
    }
}
        ...
        /// <summary>
        /// ✅ Получает значения RSI (текущее и предыдущее)
        /// </summary>
        public bool TryGetRsiValues(out decimal currentRsi, out decimal previousRsi)
        {
            currentRsi = previousRsi = 0m;

            try
            {
                if (_rsi?.DataSeries == null || _rsi.DataSeries.Count < 1)
                {
                    return false;
                }
                ...
                int lastIndex = rsiValues.Count - 1;
                currentRsi = rsiValues[lastIndex];
                previousRsi = rsiValues[lastIndex - 1];
        ...
        {
            // ✅ ОПТИМИЗАЦИЯ: Кэшируем DateTime.Now и TimeSpan для сравнения
            DateTime now = DateTime.Now;
            TimeSpan cacheExpiry = TimeSpan.FromSeconds(3);
            ...
            if (_indicatorCache.TryGetValue(indicatorKey, out CachedValue cached) && 
                now - cached.Timestamp < cacheExpiry)
            {
                return cached.Value;
            }
            ...
            try
            {
                ...
                // ✅ ОПТИМИЗАЦИЯ: Используем TryGetValue вместо ContainsKey
                // Сохраняем только валидные значения (не 0, если это не первый вызов)
                bool isFirstCall = !_indicatorCache.TryGetValue(indicatorKey, out _);
                if (value != 0m || isFirstCall)
                {
    ...
    public class RiskManagementComponent : ITradingComponent
    {
        ...
        // ✅ НОВОЕ: Флаг защиты для режима ручного TP
        private readonly ConcurrentDictionary<int, bool> _manualTPProtectionActive = new();
        ...
        public bool CanClosePosition(int positionId, decimal currentPrice, bool isLong)
        {
            try
            {
                ...
                if (currentProfitPercent < minProfitPercent)
                {
                    ...
                    // Здесь допускается ЗАКРЫТИЕ ТОЛЬКО по цене "минимальная прибыль".
                    // Дополнительное ужесточение: если по какой-то причине minProfitPrice ещё не рассчитан,
                    // ЗАПРЕЩАЕМ закрытие, чтобы не допустить выхода ниже требуемой минимальной прибыли.
                    decimal minProfitPrice = GetMinProfitPrice(positionId);
                    if (minProfitPrice <= 0)
                    {
                        _context.SendLog(
                            $"🔒 ЗАПРЕТ ЗАКРЫТИЯ ПО МИН.ПРИБЫЛИ: позиция #{positionId} текущая прибыль {currentProfitPercent:F2}% < {minProfitPercent:F2}%, " +
                            $"но уровень минимальной прибыли ещё не инициализирован (minProfitPrice=0). Закрытие заблокировано.",
                            LogMessageType.System);
                        return false;
                    }

                    _context.SendLog(
                        $"🛡️ ЗАЩИТА МИНИМАЛЬНОЙ ПРИБЫЛИ: позиция #{positionId} была в плюсе, текущая прибыль {currentProfitPercent:F2}% < {minProfitPercent:F2}%. " +
                        $"РАЗРЕШЕНО закрытие ТОЛЬКО по уровню минимальной прибыли {minProfitPrice:F4}.",
                        LogMessageType.System);

                    // ✅ РАЗРЕШАЕМ закрытие - TryClosePosition всегда скорректирует цену не ниже minProfitPrice
                    return true;
                }
        ...
        /// <summary>
        /// ✅ РАСЧЕТ ЦЕНЫ МИНИМАЛЬНОЙ ПРИБЫЛИ (РЕЖИМ "ПО ОТДЕЛЬНЫМ СДЕЛКАМ")
        /// 
        ...
        /// 
        /// ✅ КРИТИЧНО: Каждая позиция имеет свою ИНДИВИДУАЛЬНУЮ цену входа и минимальную прибыль:
        /// - Для LONG: минимальная прибыль = цена входа + (цена входа * minProfitPercent / 100)
        ///   Абсолютный запрет закрытия ниже этого уровня.
        /// - Для SHORT: минимальная прибыль = цена входа - (цена входа * minProfitPercent / 100)
        ///   Абсолютный запрет закрытия выше этого уровня.
        /// </summary>
        private decimal CalculateMinProfitPrice(decimal entryPrice, bool isLong)
        {
            ...
            // ✅ Для LONG: цена входа + минимальная прибыль
            // ✅ Для SHORT: цена входа - минимальная прибыль
            return isLong
        ...
        public decimal GetMaxProfitPercent(int positionId) => _maxProfitPercentReached.TryGetValue(positionId, out var max) ? max : 0m;
        public bool WasPositionInProfit(int positionId) => _wentPositive.TryGetValue(positionId, out var was) && was;
        ...
        /// <summary>
        /// ✅ ЗАЩИТА ЦЕНЫ ЗАКРЫТИЯ
        /// Возвращает защищенную цену закрытия, которая гарантирует минимальную прибыль.
        /// Если запрошенная цена закрытия ниже минимальной прибыли - возвращает цену минимальной прибыли.
        /// </summary>
        public decimal GetProtectedClosePrice(int positionId, decimal requestedClosePrice, bool isLong)
        {
            try
            {
                // Если позиция не инициализирована - возвращаем запрошенную цену
                if (!_entryPrices.ContainsKey(positionId))
                {
                    return requestedClosePrice;
                }

                decimal entryPrice = _entryPrices[positionId];
                decimal minProfitPrice = GetMinProfitPrice(positionId);
                
                // Если минимальная цена прибыли не рассчитана - возвращаем запрошенную цену
                if (minProfitPrice <= 0)
                {
                    return requestedClosePrice;
                }

                // ✅ Для LONG: защищенная цена = максимум из запрошенной и минимальной прибыли
                // ✅ Для SHORT: защищенная цена = минимум из запрошенной и минимальной прибыли
                decimal protectedPrice = isLong
                    ? Math.Max(requestedClosePrice, minProfitPrice)
                    : Math.Min(requestedClosePrice, minProfitPrice);

                // Логируем только если цена была скорректирована
                if (protectedPrice != requestedClosePrice)
                {
                    _context.SendLog(
                        $"🛡️ ЦЕНА ЗАКРЫТИЯ ЗАЩИЩЕНА для позиции #{positionId}: " +
                        $"Запрошена {requestedClosePrice:F4} → Защищенная {protectedPrice:F4} " +
                        $"(минимальная прибыль: {minProfitPrice:F4})",
                        LogMessageType.System);
                }
            ...
            {
                _context.SendLog($"❌ Ошибка в GetProtectedClosePrice для позиции #{positionId}: {ex.Message}", 
                    LogMessageType.Error);
                return requestedClosePrice; // В случае ошибки возвращаем запрошенную цену
            }
        }
    }
    ...
    public class PositionManagerComponent : ITradingComponent
    {
        ...
        private readonly ConcurrentDictionary<int, string> _positionCloseReasons = new(); // ✅ Причины закрытия
        ...
        private void OnPositionOpeningSuccess(Position position)
        {
            ...
            {
                int positionId = position.Number;
                // 👇 ДОБАВЬ ЭТУ СТРОКУ СРАЗУ ПОСЛЕ int positionId = position.Number;
            _context.SendLog($"🔔 ОТКРЫТИЕ ПОЗИЦИИ #{positionId} | Вход: {position.EntryPrice:F4} | " +
                         $"Время: {DateTime.Now:HH:mm:ss}", LogMessageType.System);
                _activePositions[positionId] = position;
                ...
                // ✅ КРИТИЧНО: Инициализируем экстремальные цены в TrailingStopComponent
                // Это необходимо для использования экстремальной цены при всех способах выхода
                var trailingComponent = _context.GetComponent<TrailingStopComponent>();
                if (trailingComponent != null)
                {
                    trailingComponent.InitializePosition(positionId, position.EntryPrice, position.Direction == Side.Buy);
                }
        ...
        private void OnPositionClosingSuccess(Position position)
        {
            try
            {
                ...
                // ✅ Получаем сохраненные причины открытия и закрытия для логирования
                string openReason = GetOpenReason(positionId);
                string closeReason = GetCloseReason(positionId);
                ...
                // ✅ Детальное логирование с сохраненными причинами
                string reasonInfo = $"Открытие: {openReason}";
                if (!string.IsNullOrWhiteSpace(closeReason) && closeReason != "неизвестен")
                {
                    reasonInfo += $" | Закрытие: {closeReason}";
                }
                
                _context.SendLog($"🔒 ПОЗИЦИЯ ЗАКРЫТА: #{positionId} {position.Direction} {position.SecurityName} | {reasonInfo}", 
                    LogMessageType.System);
            }
            catch (Exception ex)
            {
        ...
        private void InitializeManualPosition(Position position)
        {
            try
            {
                ...
                // ✅ КРИТИЧНО: Инициализируем экстремальные цены в TrailingStopComponent для ручных позиций
                // Это необходимо для использования экстремальной цены при всех способах выхода
                var trailingComponent = _context.GetComponent<TrailingStopComponent>();
                if (trailingComponent != null)
                {
        ...
        {
            // 1. Если причина уже сохранена и не пустая — возвращаем её
            if (_positionReasons.TryGetValue(positionId, out var reason) &&
                !string.IsNullOrWhiteSpace(reason))
            {
                return reason;
            }

            // 2. Пытаемся восстановить причину из общего хранилища сигналов
            try
            {
                if (_context != null && _activePositions.TryGetValue(positionId, out var pos))
                {
                    string key = pos.Direction == Side.Buy
                        ? SharedDataKeys.LastBuySignalType
                        : SharedDataKeys.LastSellSignalType;

                    if (_context.SharedData != null &&
                        _context.SharedData.TryGetValue(key, out var obj) &&
                        obj is string s &&
                        !string.IsNullOrWhiteSpace(s))
                    {
                        _positionReasons[positionId] = s;
                        return s;
                    }
                }
            }
            ...
            // 3. Фолбэк — причина неизвестна
            return "неизвестен";
        }
        
        /// <summary>
        /// ✅ Сохранение причины закрытия позиции для последующего использования в логах и экспорте
        /// </summary>
        public void RegisterCloseReason(int positionId, string reason)
        {
            _positionCloseReasons[positionId] = string.IsNullOrWhiteSpace(reason) ? "неизвестен" : reason;
        }
        
        /// <summary>
        /// ✅ Получение сохраненной причины закрытия позиции
        /// </summary>
        public string GetCloseReason(int positionId)
        {
            return _positionCloseReasons.TryGetValue(positionId, out var reason) && !string.IsNullOrWhiteSpace(reason)
                ? reason
                : "неизвестен";
        }
    ...
    public class TrailingStopComponent : ITradingComponent
    {
        ...
        public async Task ProcessAsync(Candle candle)
        {
            ...
            // ✅ КРИТИЧНО: Обновляем экстремумы ВСЕГДА, даже если трейлинг выключен
            // Это необходимо для использования экстремальной цены при выходе по сигналам индикаторов
            foreach (var position in activePositions)
            {
                UpdateExtremePrices(position, candle.Close);
            }
            
            // ✅ Обновляем трейлинг только если он включен
        ...
        /// <summary>
        /// ✅ Обновляет экстремальные цены (highest/lowest) для позиции
        /// Вызывается ВСЕГДА, независимо от состояния трейлинга
        /// Это необходимо для использования экстремальной цены при всех способах выхода
        /// </summary>
        private void UpdateExtremePrices(Position position, decimal currentPrice)
        {
            try
            {
                int positionId = position.Number;
                ...
                // ✅ Обновляем экстремумы ВСЕГДА (независимо от трейлинга)
                if (isLong)
                {
        ...
        public void ActivateTrailing(int positionId, bool isLong, decimal currentPrice)
        {
            ...
            // ✅ ИСПРАВЛЕНО: При активации используем currentPrice как basePrice (экстремальная цена еще не установлена)
            // Но сразу обновляем экстремумы, чтобы следующий расчет был от экстремума
            decimal extremePrice = isLong 
                ? _highestPricesSinceEntry.GetOrAdd(positionId, entryPrice)
                : _lowestPricesSinceEntry.GetOrAdd(positionId, entryPrice);
            decimal initialTrailingLevel = CalculateTrailingLevel(positionId, extremePrice, currentPrice, isLong);
        ...
        // ✅ ЭТАЛОННАЯ ЛОГИКА ИЗ ПРОВЕРЕННОГО ФАЙЛА АНДРЕЯ
        public bool CheckTrailingStop(int positionId, decimal currentPrice, Position position)
        {
            try
            {
                ...
                // ✅ ЭТАЛОН: Рассчитываем прибыль
                decimal profitPercent = isLong 
                    ? (currentPrice - entryPrice) / entryPrice * 100m
                    : (entryPrice - currentPrice) / entryPrice * 100m;
                
                // ✅ ЭТАЛОН: Проверяем условие активации трейлинга
                decimal trailingStart = _trailingStartPercent?.ValueDecimal ?? 0m;
                if (profitPercent >= trailingStart)
                {
                    // ✅ ИСПРАВЛЕНО: Рассчитываем уровень трейлинга от ЭКСТРЕМАЛЬНОЙ цены (как у Андрея)
                    decimal extremePrice = isLong 
                        ? _highestPricesSinceEntry.GetOrAdd(positionId, entryPrice)
                        : _lowestPricesSinceEntry.GetOrAdd(positionId, entryPrice);
                    decimal trailingLevel = CalculateTrailingLevel(positionId, extremePrice, currentPrice, isLong);
                    
                    // ✅ ЭТАЛОН: Ограничиваем минимальной прибылью
                    decimal minProfitPrice = riskManager.GetMinProfitPrice(positionId);
                    if (minProfitPrice > 0)
                    {
                        if (isLong && trailingLevel < minProfitPrice) trailingLevel = minProfitPrice;
                        if (!isLong && trailingLevel > minProfitPrice) trailingLevel = minProfitPrice;
                    }
                    
                    // ✅ ЭТАЛОН: Обновляем уровень только если новый лучше старого
                    ...
                    // ✅ ЭТАЛОН: Проверяем срабатывание стопа
                    decimal activeTrailingLevel = _currentTrailingLevels.ContainsKey(positionId) 
                        ? _currentTrailingLevels[positionId] 
                        : trailingLevel;
                    bool stopHit = isLong ? currentPrice <= activeTrailingLevel 
                                         : currentPrice >= activeTrailingLevel;
                    
                    // Детальное логирование при срабатывании трейлинг-стопа
                    if (stopHit)
                    {
                        decimal finalProfitPercent = isLong 
                            ? (activeTrailingLevel - entryPrice) / entryPrice * 100m
                            : (entryPrice - activeTrailingLevel) / entryPrice * 100m;
                        
                        decimal distanceToLevel = isLong 
                            ? (currentPrice - activeTrailingLevel) 
                            : (activeTrailingLevel - currentPrice);
                        ...
                        _context.SendLog($"   💰 Текущая цена: {currentPrice:F4} | Уровень трейлинга: {activeTrailingLevel:F4}", LogMessageType.Trade);
                        _context.SendLog($"   📊 Прибыль при срабатывании: {finalProfitPercent:F2}% | Расстояние до уровня: {distancePercent:F3}%", LogMessageType.Trade);
                    }
        ...
        private void CheckAndUpdateTrailing(Position position, decimal currentPrice)
        {
            try
            {
                ...
                // ✅ ИСПРАВЛЕНО: ЛОГИКА КАК У АНДРЕЯ (ЭТАЛОН) - ИСПОЛЬЗУЕМ ЭКСТРЕМАЛЬНУЮ ЦЕНУ
                // Экстремумы уже обновлены в UpdateExtremePrices
                // ✅ КРИТИЧНО: Трейлинг рассчитывается от ЭКСТРЕМАЛЬНОЙ цены (максимум для LONG, минимум для SHORT)
                // Это позволяет стопу "следовать" за максимумом движения, а не возвращаться назад при откатах

                // Получаем экстремальную цену (максимум для LONG, минимум для SHORT)
                decimal extremePrice = isLong 
                    ? _highestPricesSinceEntry.GetOrAdd(positionId, entryPrice)
                    : _lowestPricesSinceEntry.GetOrAdd(positionId, entryPrice);

                // Проверяем условие активации трейлинга (используем текущую цену для проверки активации)
                decimal profitPercent = isLong
                ...
                if (profitPercent >= trailingStart)
                {
                    ...
                    // ✅ ИСПРАВЛЕНО: ЛОГИКА КАК У АНДРЕЯ (ЭТАЛОН) - ИСПОЛЬЗУЕМ ЭКСТРЕМАЛЬНУЮ ЦЕНУ
                    // ✅ КРИТИЧНО: Трейлинг рассчитывается от ЭКСТРЕМАЛЬНОЙ цены (extremePrice)
                    // Это позволяет стопу "следовать" за максимумом движения, а не возвращаться назад при откатах
                    // Уровень обновляется только если новый уровень лучше старого
                    decimal trailingLevel = CalculateTrailingLevel(positionId, extremePrice, currentPrice, isLong);
                    ...
                    // ✅ ЛОГИКА КАК У АНДРЕЯ: обновляем только если новый уровень лучше старого
                    // Это ключевой момент - стоп "следует" за ценой, но не возвращается назад
                    if (!_currentTrailingLevels.ContainsKey(positionId))
                    {
                    ...
                    {
                        // ✅ КАК У АНДРЕЯ: обновляем только если новый уровень лучше (для LONG выше, для SHORT ниже)
                        bool isBetter = isLong 
                            ? trailingLevel > oldLevel  // Для LONG: новый уровень выше старого
                            : trailingLevel < oldLevel; // Для SHORT: новый уровень ниже старого
        ...
        // ✅ ИСПРАВЛЕНО: ЛОГИКА КАК У АНДРЕЯ (ЭТАЛОН) - ИСПОЛЬЗУЕМ ЭКСТРЕМАЛЬНУЮ ЦЕНУ
        // basePrice = extremePrice (экстремальная цена: максимум для LONG, минимум для SHORT)
        // currentPrice используется для расчета прибыли в самообучаемом трейлинге (для адаптации дистанции)
        private decimal CalculateTrailingLevel(int positionId, decimal basePrice, decimal currentPrice, bool isLong)
        {
            if (_trailingType.ValueString == "ATR")
            {
                ...
                // ✅ ИСПОЛЬЗУЕМ basePrice (экстремальную цену) для расчета уровня трейлинга
                // Стоп рассчитывается от максимума/минимума, а не от текущей цены
                return isLong 
                    ? basePrice - atr * _atrMultiplier.ValueDecimal
                    : basePrice + atr * _atrMultiplier.ValueDecimal;
            }
            else if (_trailingType.ValueString == "Самообучаемый")
            {
                // ✅ САМООБУЧАЕМЫЙ ТРЕЙЛИНГ: Адаптивный расчет на основе исторических данных
                // ✅ ПЕРЕДАЕМ basePrice (экстремальную цену) для расчета уровня трейлинга
                // ✅ ПЕРЕДАЕМ currentPrice для правильного расчета прибыли (для адаптации дистанции)
                return CalculateSelfLearningTrailingLevel(positionId, basePrice, currentPrice, isLong);
            }
            else
            {
                // ✅ ИСПОЛЬЗУЕМ basePrice (экстремальную цену) для фиксированного трейлинга
                // Стоп всегда на X% ниже максимума (для LONG) или выше минимума (для SHORT)
                return isLong 
                    ? basePrice * (1 - _trailingDistancePercent.ValueDecimal / 100m)
                    : basePrice * (1 + _trailingDistancePercent.ValueDecimal / 100m);
            }
        }
        ...
        /// 
        /// 5. ПРИМЕНЕНИЕ: Рассчитанная дистанция применяется к ЭКСТРЕМАЛЬНОЙ цене (basePrice)
        ///    - Для LONG: trailingLevel = basePrice * (1 - адаптивная_дистанция / 100)
        ///    - Для SHORT: trailingLevel = basePrice * (1 + адаптивная_дистанция / 100)
        ///    - basePrice = highest для LONG, lowest для SHORT (максимум/минимум с момента входа)
        /// </summary>
        // ✅ ИСПРАВЛЕНО: ЛОГИКА КАК У АНДРЕЯ (ЭТАЛОН) - ИСПОЛЬЗУЕМ ЭКСТРЕМАЛЬНУЮ ЦЕНУ
        // basePrice = extremePrice (экстремальная цена: максимум для LONG, минимум для SHORT)
        // currentPrice используется для расчета прибыли (для адаптации дистанции)
        private decimal CalculateSelfLearningTrailingLevel(int positionId, decimal basePrice, decimal currentPrice, bool isLong)
        {
            try
            {
                var riskManager = _context.GetComponent<RiskManagementComponent>();
                ...
                // ✅ ИСПРАВЛЕНО: Прибыль для адаптации от ЭКСТРЕМАЛЬНОЙ цены (basePrice)
                // Адаптация на максимуме движения, а не на откатах (логика Андрея)
                    decimal currentProfitPercent = isLong
                    ? (basePrice - entryPrice) / entryPrice * 100m
                    : (entryPrice - basePrice) / entryPrice * 100m;
                ...
                // ✅ Базовая дистанция из параметров (используем параметр самообучаемого или обычный)
                decimal baseDistance = 0.1m;
                if (_context.SharedData.TryGetValue("SelfLearningBaseDistance", out var baseDistObj)
                    && baseDistObj is StrategyParameterDecimal baseDistParam)
                {
                    baseDistance = baseDistParam.ValueDecimal;
                }
                else
                {
                    baseDistance = _trailingDistancePercent?.ValueDecimal ?? 0.1m;
                }
                
                // ✅ Анализ исторических данных для адаптации с использованием параметров
                decimal adaptiveMultiplier = 1.0m;
                
                // Получаем параметры порогов из SharedData или используем значения по умолчанию
                decimal profitThresholdHigh = 1.2m;
                decimal profitThresholdLow = 0.8m;
                decimal volatilityThresholdHigh = 1.5m;
                decimal volatilityThresholdLow = 0.5m;
                decimal highProfitMultiplier = 1.3m;
                decimal lowProfitMultiplier = 0.8m;
                decimal highVolatilityMultiplier = 1.2m;
                decimal lowVolatilityMultiplier = 0.9m;
                
                if (_context.SharedData.TryGetValue("SelfLearningProfitThresholdHigh", out var pthObj)
                    && pthObj is StrategyParameterDecimal pthParam)
                    profitThresholdHigh = pthParam.ValueDecimal;
                if (_context.SharedData.TryGetValue("SelfLearningProfitThresholdLow", out var ptlObj)
                    && ptlObj is StrategyParameterDecimal ptlParam)
                    profitThresholdLow = ptlParam.ValueDecimal;
                if (_context.SharedData.TryGetValue("SelfLearningVolatilityThresholdHigh", out var vthObj)
                    && vthObj is StrategyParameterDecimal vthParam)
                    volatilityThresholdHigh = vthParam.ValueDecimal;
                if (_context.SharedData.TryGetValue("SelfLearningVolatilityThresholdLow", out var vtlObj)
                    && vtlObj is StrategyParameterDecimal vtlParam)
                    volatilityThresholdLow = vtlParam.ValueDecimal;
                if (_context.SharedData.TryGetValue("SelfLearningHighProfitMultiplier", out var hpmObj)
                    && hpmObj is StrategyParameterDecimal hpmParam)
                    highProfitMultiplier = hpmParam.ValueDecimal;
                if (_context.SharedData.TryGetValue("SelfLearningLowProfitMultiplier", out var lpmObj)
                    && lpmObj is StrategyParameterDecimal lpmParam)
                    lowProfitMultiplier = lpmParam.ValueDecimal;
                if (_context.SharedData.TryGetValue("SelfLearningHighVolatilityMultiplier", out var hvmObj)
                    && hvmObj is StrategyParameterDecimal hvmParam)
                    highVolatilityMultiplier = hvmParam.ValueDecimal;
                if (_context.SharedData.TryGetValue("SelfLearningLowVolatilityMultiplier", out var lvmObj)
                    && lvmObj is StrategyParameterDecimal lvmParam)
                    lowVolatilityMultiplier = lvmParam.ValueDecimal;
                
                // Получаем размер истории из параметров
                int historySize = 50;
                if (_context.SharedData.TryGetValue("SelfLearningHistorySize", out var hsObj)
                    && hsObj is StrategyParameterInt hsParam)
                    historySize = hsParam.ValueInt;
                
                // Ограничиваем размер истории
                if (_historicalProfits[positionId].Count > historySize)
                {
                    int removeCount = _historicalProfits[positionId].Count - historySize;
                    _historicalProfits[positionId].RemoveRange(0, removeCount);
                    _historicalVolatilities[positionId].RemoveRange(0, removeCount);
                }
                ...
                if (_historicalProfits[positionId].Count >= 5)
                {
                    ...
                    // ✅ Если текущая прибыль выше порога → увеличиваем дистанцию (даем больше пространства)
                    if (currentProfitPercent > avgProfit * profitThresholdHigh)
                    {
                        adaptiveMultiplier = highProfitMultiplier;
                    }
                    // ✅ Если текущая прибыль ниже порога → уменьшаем дистанцию (защищаем прибыль)
                    else if (currentProfitPercent < avgProfit * profitThresholdLow)
                    {
                        adaptiveMultiplier = lowProfitMultiplier;
                    }
                    
                    // ✅ Учет волатильности: высокая волатильность → большая дистанция
                    if (volatilityPercent > avgVolatility * volatilityThresholdHigh)
                    {
                        adaptiveMultiplier *= highVolatilityMultiplier;
                    }
                    else if (volatilityPercent < avgVolatility * volatilityThresholdLow)
                    {
                        adaptiveMultiplier *= lowVolatilityMultiplier;
                    }
                }
                ...
                // ✅ Ограничиваем дистанцию параметрами (или значениями по умолчанию)
                decimal minDistance = 0.05m;
                decimal maxDistance = 5.0m;
                if (_context.SharedData.TryGetValue("SelfLearningMinDistance", out var minDistObj)
                    && minDistObj is StrategyParameterDecimal minDistParam)
                    minDistance = minDistParam.ValueDecimal;
                if (_context.SharedData.TryGetValue("SelfLearningMaxDistance", out var maxDistObj)
                    && maxDistObj is StrategyParameterDecimal maxDistParam)
                    maxDistance = maxDistParam.ValueDecimal;
                
                adaptiveDistance = Math.Max(minDistance, Math.Min(maxDistance, adaptiveDistance));
                
                // ✅ ИСПРАВЛЕНО: ЛОГИКА КАК У АНДРЕЯ (ЭТАЛОН) - ИСПОЛЬЗУЕМ ЭКСТРЕМАЛЬНУЮ ЦЕНУ
                // ✅ ВАЖНО: Расчет уровня трейлинга происходит от basePrice (экстремальной цены)
                // Это ключевой момент: мы фиксируем стоп не от текущей цены, а от пика,
                // позволяя цене откатиться, но не пробить стоп, установленный от максимума.
                // Уровень обновляется только если новый лучше старого (логика в CheckAndUpdateTrailing)
                decimal trailingLevel = isLong 
                    ? basePrice * (1 - adaptiveDistance / 100m)
                    : basePrice * (1 + adaptiveDistance / 100m);
            ...
            catch (Exception ex)
            {
                ...
                // ✅ Fallback тоже использует basePrice (экстремальную цену)
                return isLong 
                    ? basePrice * (1 - (_trailingDistancePercent?.ValueDecimal ?? 0.1m) / 100m)
                    : basePrice * (1 + (_trailingDistancePercent?.ValueDecimal ?? 0.1m) / 100m);
            }
        }
        ...
        /// <summary>
        /// ✅ Получает экстремальную цену (highest для LONG, lowest для SHORT) с момента входа
        /// </summary>
        public decimal GetExtremePrice(int positionId, bool isLong)
        {
            if (isLong)
            {
                return _highestPricesSinceEntry.TryGetValue(positionId, out var highest) ? highest : 0m;
            }
            else
            {
                return _lowestPricesSinceEntry.TryGetValue(positionId, out var lowest) ? lowest : 0m;
            }
        }
        
        /// <summary>
        /// ✅ Получает максимальную цену с момента входа (для LONG)
        /// </summary>
        public decimal GetHighestPrice(int positionId)
        {
            return _highestPricesSinceEntry.TryGetValue(positionId, out var highest) ? highest : 0m;
        }
        
        /// <summary>
        /// ✅ Получает минимальную цену с момента входа (для SHORT)
        /// </summary>
        public decimal GetLowestPrice(int positionId)
        {
            return _lowestPricesSinceEntry.TryGetValue(positionId, out var lowest) ? lowest : 0m;
        }
        
        /// <summary>
        /// ✅ Инициализирует экстремальные цены при открытии позиции
        /// Вызывается при открытии позиции для правильной инициализации экстремумов
        /// </summary>
        public void InitializePosition(int positionId, decimal entryPrice, bool isLong)
        {
            try
            {
                if (isLong)
    ...
    public class EnhancedAIOptimizationComponent : ITradingComponent
    {
        ...
        private StrategyParameterBool _optimizeRsiPeriod;
        private StrategyParameterBool _optimizeRsiOversold;
        private StrategyParameterBool _optimizeRsiOverbought;
        ...
        // ✅ Флаги оптимизации параметров самообучаемого трейлинга
        private StrategyParameterBool _optimizeSelfLearningBaseDistance;
        private StrategyParameterBool _optimizeSelfLearningHighProfitMultiplier;
        private StrategyParameterBool _optimizeSelfLearningLowProfitMultiplier;
        private StrategyParameterBool _optimizeSelfLearningHighVolatilityMultiplier;
        private StrategyParameterBool _optimizeSelfLearningLowVolatilityMultiplier;
        private StrategyParameterBool _optimizeSelfLearningMinDistance;
        private StrategyParameterBool _optimizeSelfLearningMaxDistance;
        private StrategyParameterBool _optimizeSelfLearningHistorySize;
        private StrategyParameterBool _optimizeSelfLearningProfitThresholdHigh;
        private StrategyParameterBool _optimizeSelfLearningProfitThresholdLow;
        private StrategyParameterBool _optimizeSelfLearningVolatilityThresholdHigh;
        private StrategyParameterBool _optimizeSelfLearningVolatilityThresholdLow;
        ...
        public void Initialize(IComponentContext context)
        {
            ...
            if (context.SharedData.TryGetValue("OptimizeRsiPeriod", out var optRsiPeriod))
                _optimizeRsiPeriod = optRsiPeriod as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeRsiOversold", out var optRsiOs))
                _optimizeRsiOversold = optRsiOs as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeRsiOverbought", out var optRsiOb))
                _optimizeRsiOverbought = optRsiOb as StrategyParameterBool;
            ...
            // ✅ Инициализация флагов оптимизации параметров самообучаемого трейлинга
            if (context.SharedData.TryGetValue("OptimizeSelfLearningBaseDistance", out var optSLBaseDist))
                _optimizeSelfLearningBaseDistance = optSLBaseDist as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeSelfLearningHighProfitMultiplier", out var optSLHPM))
                _optimizeSelfLearningHighProfitMultiplier = optSLHPM as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeSelfLearningLowProfitMultiplier", out var optSLLPM))
                _optimizeSelfLearningLowProfitMultiplier = optSLLPM as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeSelfLearningHighVolatilityMultiplier", out var optSLHVM))
                _optimizeSelfLearningHighVolatilityMultiplier = optSLHVM as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeSelfLearningLowVolatilityMultiplier", out var optSLLVM))
                _optimizeSelfLearningLowVolatilityMultiplier = optSLLVM as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeSelfLearningMinDistance", out var optSLMinDist))
                _optimizeSelfLearningMinDistance = optSLMinDist as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeSelfLearningMaxDistance", out var optSLMaxDist))
                _optimizeSelfLearningMaxDistance = optSLMaxDist as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeSelfLearningHistorySize", out var optSLHistSize))
                _optimizeSelfLearningHistorySize = optSLHistSize as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeSelfLearningProfitThresholdHigh", out var optSLPTH))
                _optimizeSelfLearningProfitThresholdHigh = optSLPTH as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeSelfLearningProfitThresholdLow", out var optSLPTL))
                _optimizeSelfLearningProfitThresholdLow = optSLPTL as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeSelfLearningVolatilityThresholdHigh", out var optSLVTH))
                _optimizeSelfLearningVolatilityThresholdHigh = optSLVTH as StrategyParameterBool;
            if (context.SharedData.TryGetValue("OptimizeSelfLearningVolatilityThresholdLow", out var optSLVTL))
                _optimizeSelfLearningVolatilityThresholdLow = optSLVTL as StrategyParameterBool;
        ...
        private EnhancedPSOConfiguration CreateEnhancedConfiguration()
        {
            ...
            // ✅ RSI параметры оптимизации
            if (_optimizeRsiPeriod != null && _optimizeRsiPeriod.ValueBool == true)
                config.ParameterRanges["RsiPeriod"] = new ParameterRange(5, 50, true);
            if (_optimizeRsiOversold != null && _optimizeRsiOversold.ValueBool == true)
                config.ParameterRanges["RsiOversold"] = new ParameterRange(10m, 50m);
            if (_optimizeRsiOverbought != null && _optimizeRsiOverbought.ValueBool == true)
                config.ParameterRanges["RsiOverbought"] = new ParameterRange(50m, 90m);
            ...
            // ✅ Параметры самообучаемого трейлинга
            if (_optimizeSelfLearningBaseDistance != null && _optimizeSelfLearningBaseDistance.ValueBool == true)
                config.ParameterRanges["SelfLearningBaseDistance"] = new ParameterRange(0.05m, 5.0m);
            if (_optimizeSelfLearningHighProfitMultiplier != null && _optimizeSelfLearningHighProfitMultiplier.ValueBool == true)
                config.ParameterRanges["SelfLearningHighProfitMultiplier"] = new ParameterRange(0.5m, 3.0m);
            if (_optimizeSelfLearningLowProfitMultiplier != null && _optimizeSelfLearningLowProfitMultiplier.ValueBool == true)
                config.ParameterRanges["SelfLearningLowProfitMultiplier"] = new ParameterRange(0.3m, 1.5m);
            if (_optimizeSelfLearningHighVolatilityMultiplier != null && _optimizeSelfLearningHighVolatilityMultiplier.ValueBool == true)
                config.ParameterRanges["SelfLearningHighVolatilityMultiplier"] = new ParameterRange(0.5m, 3.0m);
            if (_optimizeSelfLearningLowVolatilityMultiplier != null && _optimizeSelfLearningLowVolatilityMultiplier.ValueBool == true)
                config.ParameterRanges["SelfLearningLowVolatilityMultiplier"] = new ParameterRange(0.3m, 1.5m);
            if (_optimizeSelfLearningMinDistance != null && _optimizeSelfLearningMinDistance.ValueBool == true)
                config.ParameterRanges["SelfLearningMinDistance"] = new ParameterRange(0.01m, 1.0m);
            if (_optimizeSelfLearningMaxDistance != null && _optimizeSelfLearningMaxDistance.ValueBool == true)
                config.ParameterRanges["SelfLearningMaxDistance"] = new ParameterRange(1.0m, 10.0m);
            if (_optimizeSelfLearningHistorySize != null && _optimizeSelfLearningHistorySize.ValueBool == true)
                config.ParameterRanges["SelfLearningHistorySize"] = new ParameterRange(10, 200, true);
            if (_optimizeSelfLearningProfitThresholdHigh != null && _optimizeSelfLearningProfitThresholdHigh.ValueBool == true)
                config.ParameterRanges["SelfLearningProfitThresholdHigh"] = new ParameterRange(1.0m, 2.0m);
            if (_optimizeSelfLearningProfitThresholdLow != null && _optimizeSelfLearningProfitThresholdLow.ValueBool == true)
                config.ParameterRanges["SelfLearningProfitThresholdLow"] = new ParameterRange(0.5m, 1.0m);
            if (_optimizeSelfLearningVolatilityThresholdHigh != null && _optimizeSelfLearningVolatilityThresholdHigh.ValueBool == true)
                config.ParameterRanges["SelfLearningVolatilityThresholdHigh"] = new ParameterRange(1.0m, 3.0m);
            if (_optimizeSelfLearningVolatilityThresholdLow != null && _optimizeSelfLearningVolatilityThresholdLow.ValueBool == true)
                config.ParameterRanges["SelfLearningVolatilityThresholdLow"] = new ParameterRange(0.1m, 1.0m);
        ...
        {
            // ✅ УПРОЩЕННОЕ ЛОГИРОВАНИЕ: одна строка
            _context.SendLog(
                $"🚀 Оптимизация: PSO {config.SwarmSize} частиц × {config.MaxIterations} итераций | " +
                $"GA {config.GAPopulationSize} популяция × {config.GAGenerations} поколений | " +
                $"Параметров: {config.ParameterRanges.Count}",
                LogMessageType.System);
        }
        ...
        private bool CheckMarketConditions()
        {
            try
            {
                ...
                if (candles == null || candles.Count < 50) return false;
                
                // ✅ ОПТИМИЗАЦИЯ: Используем прямой доступ к последним свечам вместо TakeLast
                int candleCount = candles.Count;
                int startIndex = Math.Max(0, candleCount - 20);
                int count = candleCount - startIndex;
                ...
                for (int i = startIndex; i < candleCount; i++)
                {
                    var candle = candles[i];
        ...
        private void InitializeEnhancedSwarm(EnhancedPSOConfiguration config)
        {
            ...
            // ✅ ОПТИМИЗАЦИЯ: Кэшируем ParameterRanges и размер роя
            var parameterRanges = config.ParameterRanges;
            int swarmSize = config.SwarmSize;
            int swarmSizeThird = swarmSize / 3;
            int swarmSizeTwoThirds = 2 * swarmSize / 3;
            ...
            for (int i = 0; i < swarmSize; i++)
            {
                ...
                foreach (var paramRange in parameterRanges)
                {
                    string paramKey = paramRange.Key;
                    var range = paramRange.Value; // ✅ ОПТИМИЗАЦИЯ: Кэшируем range
                    decimal randomValue = GenerateRandomValue(range);
                    
                    // ✅ ОСОБАЯ ИНИЦИАЛИЗАЦИЯ ДЛЯ TenkanLength: более широкий разброс
                    if (paramKey == "TenkanLength")
                    {
                    ...
                    particle.Position[paramKey] = randomValue;
                    ...
                    decimal rangeSize = range.MaxValue - range.MinValue;
                    decimal initialVelocity = (decimal)(_random.NextDouble() * 0.5 - 0.25) * rangeSize * 0.01m;
                    particle.Velocity[paramKey] = initialVelocity;
                    particle.PersonalBestPosition[paramKey] = randomValue;
                }
                
                _currentSwarm.Particles.Add(particle);
                
                // Логирование образца частицы убрано для упрощения
            }
            ...
            // ✅ УПРОЩЕННОЕ ЛОГИРОВАНИЕ: одна строка
            _context.SendLog($"🐝 Рой инициализирован: {swarmSize} частиц", LogMessageType.System);
        }
        ...
        // ✅ ОПТИМИЗАЦИЯ: Кэшируем список важных параметров
        private static readonly string[] _importantParams = new[] 
        { 
            "TenkanLength", 
            "KijunLength", 
            "SenkouBLength",
        ...
        {
            // ✅ ОПТИМИЗАЦИЯ: Находим лучшую частицу за один проход без сортировки
            ...
            {
                double fitness = particle.CurrentFitness;
            ...
            var averageFitness = _currentSwarm.Particles.Count > 0 
                ? totalFitness / _currentSwarm.Particles.Count 
                : 0;
            
            double progress = maxIterations == 0 ? 0 : (double)iteration / maxIterations * 100.0;
            
            // ✅ УПРОЩЕННОЕ ЛОГИРОВАНИЕ: только одна строка прогресса
            _context.SendLog(
                $"⚡ PSO: {progress:F1}% | Лучший: {_currentSwarm.GlobalBestFitness:F2}% | Средний: {averageFitness:F2}%",
                LogMessageType.System);
        }
        
        private double CalculateEnhancedDiversity()
        {
            int particleCount = _currentSwarm.Particles.Count;
            if (particleCount < 2) return 0;
            
            try
            {
                // ✅ ОПТИМИЗАЦИЯ: Рассчитываем разнообразие по всем параметрам без LINQ
                ...
                {
                    // ✅ ОПТИМИЗАЦИЯ: Собираем значения за один проход без LINQ
                    var values = new List<double>(particleCount);
                    foreach (var particle in _currentSwarm.Particles)
                    {
                        if (particle.Position.TryGetValue(paramKey, out decimal value))
                        {
                    ...
                    if (valueCount > 1)
                    {
                        // ✅ ОПТИМИЗАЦИЯ: Вычисляем среднее и дисперсию за один проход
                        double sum = 0;
                        foreach (var val in values)
                        {
                            sum += val;
                        }
                        double mean = sum / valueCount;
                        ...
                        double variance = sumSquaredDiff / valueCount;
                        double stdDev = Math.Sqrt(variance);
                        double diversity = stdDev / (mean == 0 ? 1 : mean);
        ...
        {
            // ✅ ОПТИМИЗАЦИЯ: Кэшируем DateTime.Now один раз для всех частиц
            ...
            foreach (var particle in _currentSwarm.Particles)
            {
                if (particle.CurrentFitness > _currentSwarm.GlobalBestFitness)
                {
                    ...
                    _currentSwarm.LastImprovementTime = currentTime;
                    
                    _currentReport.ImprovementHistory.Add(
                        $"PSO Глобальное улучшение (Итерация {_currentSwarm.Iteration}): +{improvement:F3}%");
                    
                    // ✅ ОПТИМИЗАЦИЯ: Используем TryGetValue вместо ContainsKey + indexer
        ...
        // ✅ ОПТИМИЗАЦИЯ: Кэшируем защищенные параметры и массивы для логирования
        private static readonly HashSet<string> _protectedParams = new HashSet<string> 
        { 
            "MinProfitPercent", 
            "MaxSpreadPercent",
        ...
        private static readonly HashSet<string> _ichimokuParams = new HashSet<string> 
        { 
            "TenkanLength", "KijunLength", "SenkouBLength", "SenkouOffset" 
        };
        
        private void UpdateParticles(EnhancedPSOConfiguration config, double inertia)
        {
            // ✅ ОПТИМИЗАЦИЯ: Кэшируем ParameterRanges и проверку защиты один раз
            var parameterRanges = config.ParameterRanges;
            bool preserveSafety = _preserveSafetyLogic?.ValueBool == true;
            bool shouldLogDetails = _currentSwarm.Iteration < 3;
            ...
            foreach (var particle in _currentSwarm.Particles)
            {
                ...
                foreach (var paramRange in parameterRanges)
                {
                    ...
                    {
                        particle.Position[param] = GenerateRandomValue(range);
                    ...
                    if (preserveSafety && _protectedParams.Contains(param))
                    {
                    ...
                    try
                    {
                        ...
                        if (shouldLogDetails && _ichimokuParams.Contains(param))
                        {
        ...
        private bool CheckEnhancedConvergence(EnhancedPSOConfiguration config, int iteration)
        {
            ...
            // ✅ ОПТИМИЗАЦИЯ: Кэшируем DateTime.Now и пороговые значения
            DateTime currentTime = DateTime.Now;
            double maxIterationsThreshold = config.MaxIterations * 0.7;
            double maxIterationsThreshold60 = config.MaxIterations * 0.6;
            double maxIterationsThreshold50 = config.MaxIterations * 0.5;
            
            // Проверяем разнообразие
            double diversity = CalculateEnhancedDiversity();
            if (diversity < 0.01 && iteration > maxIterationsThreshold)
            {
            ...
            // ✅ ОПТИМИЗАЦИЯ: Проверяем улучшения за последние итерации - используем индексы вместо TakeLast
            int historyCount = _currentSwarm.FitnessHistory.Count;
            if (historyCount >= 15)
            {
                // ✅ ОПТИМИЗАЦИЯ: Прямой доступ к последним элементам вместо TakeLast
                int startIndex = historyCount - 15;
                double maxRecent = double.MinValue;
                double firstRecent = _currentSwarm.FitnessHistory[startIndex];
                
                for (int i = startIndex; i < historyCount; i++)
                {
                    double value = _currentSwarm.FitnessHistory[i];
                    if (value > maxRecent) maxRecent = value;
                }
                
                double improvement = maxRecent - firstRecent;
                
                if (improvement < 0.01 && iteration > maxIterationsThreshold60)
                {
            ...
            if ((currentTime - _currentSwarm.LastImprovementTime).TotalMinutes > 5 && 
                iteration > maxIterationsThreshold50)
            {
        ...
        private void LogGAStatus(int generation, int maxGenerations)
        {
            ...
            _context.SendLog(
                $"🧬 GA: {progress:F1}% | Лучший: {_geneticAlgorithm.BestFitness:F2}%",
                LogMessageType.System);
        }
        ...
        {
            int populationCount = _geneticAlgorithm.Population.Count;
            tournamentSize = Math.Min(tournamentSize, populationCount);
            
            // ✅ ОПТИМИЗАЦИЯ: Находим лучшую хромосому за один проход без создания списка и сортировки
            Dictionary<string, decimal> best = null;
            double bestFitness = double.MinValue;
            
            for (int i = 0; i < tournamentSize; i++)
            {
                int index = _random.Next(populationCount);
                var candidate = _geneticAlgorithm.Population[index];
                
                var paramKey = GetEnhancedParametersKey(candidate);
                double fitness = _fitnessCache.TryGetValue(paramKey, out double cachedFitness) 
                    ? cachedFitness 
                    : -1000;
            ...
            return best ?? _geneticAlgorithm.Population[0];
        }
        ...
        private bool CheckGAConvergence(EnhancedPSOConfiguration config, int generation)
        {
            ...
            if (generation > config.GAGenerations * 0.7)
            {
        ...
        // ✅ ОПТИМИЗАЦИЯ: Кэш для ключей параметров и StringBuilder для построения строк
        private readonly Dictionary<string, string> _parametersKeyCache = new Dictionary<string, string>();
        private readonly System.Text.StringBuilder _keyBuilder = new System.Text.StringBuilder(256);
        
        private string GetEnhancedParametersKey(Dictionary<string, decimal> parameters)
        {
            // ✅ ОПТИМИЗАЦИЯ: Используем StringBuilder вместо string.Join для лучшей производительности
            _keyBuilder.Clear();
            bool first = true;
            
            // ✅ ОПТИМИЗАЦИЯ: Сортируем только ключи, затем обращаемся к словарю напрямую
            var sortedKeys = parameters.Keys.OrderBy(k => k);
            foreach (var key in sortedKeys)
            {
                if (!first) _keyBuilder.Append('|');
                _keyBuilder.Append(key).Append(':').Append(parameters[key].ToString("F6"));
                first = false;
            }
        ...
        private void GenerateEnhancedReport(HybridOptimizationResult result, EnhancedPSOConfiguration config)
        {
            ...
            // ✅ УПРОЩЕННЫЙ ОТЧЕТ: одна строка с результатами и одна строка с лучшими параметрами
            double progressPercent = result.PSOIterations > 0 
                ? (double)result.PSOIterations / config.MaxIterations * 100 
                : 100;
            
            // Рассчитываем годовую доходность в рублях (примерно, если есть данные)
            decimal annualReturnRub = (decimal)report.BestFitness; // Упрощённо: используем процент как базовую метрику
            
            _context.SendLog(
                $"✅ ОПТИМИЗАЦИЯ ЗАВЕРШЕНА: {progressPercent:F1}% | Лучшая доходность: {report.BestFitness:F2}% годовых | Время: {result.OptimizationTime:hh\\:mm\\:ss}",
                LogMessageType.System);
            
            // Одна строка с лучшими параметрами индикаторов
            var bestParams = result.BestParameters;
            if (bestParams != null && bestParams.Count > 0)
            {
                var ichimokuKeys = new[] { "TenkanLength", "KijunLength", "SenkouBLength", "SenkouOffset" };
                var paramStrings = ichimokuKeys
                    .Where(k => bestParams.ContainsKey(k))
                    .Select(k => $"{k}={bestParams[k]:F0}")
                    .ToList();
                
                if (paramStrings.Count > 0)
                {
                    _context.SendLog(
                        $"🏆 Лучшие параметры: {string.Join(" | ", paramStrings)}",
                        LogMessageType.System);
                }
            }
        }
        ...
        private void ApplyOptimizedParameters(Dictionary<string, decimal> parameters)
        {
            try
            {
                ...
                {
                    if (sharedData.TryGetValue(param.Key, out var strategyParam))
                    {
                ...
                // ✅ УПРОЩЕННОЕ ЛОГИРОВАНИЕ: одна строка
                _context.SendLog($"✅ Параметры применены: {appliedCount} из {parameters.Count}", LogMessageType.System);
            }
            catch (Exception ex)
            {
    ...
    public class IchimokuStrategyComponent : ITradingComponent
    {
        ...
        private bool? _cachedOpenByRsi;
        private bool? _cachedExitByRsi;
        ...
        private StrategyParameterString _openByRsi;
        private StrategyParameterString _exitByRsi;
        private StrategyParameterString _useCounterintuitive;
        ...
        public void Initialize(IComponentContext context)
        {
            ...
            if (context.SharedData.TryGetValue("OpenByRsi", out var openByRsi))
                _openByRsi = openByRsi as StrategyParameterString;
            if (context.SharedData.TryGetValue("ExitByRsi", out var exitByRsi))
                _exitByRsi = exitByRsi as StrategyParameterString;
            if (context.SharedData.TryGetValue("StochPeriod", out var stochPeriod))
                _stochPeriod = stochPeriod as StrategyParameterInt;
            if (context.SharedData.TryGetValue("StochSmoothing", out var stochSmooth))
                _stochSmoothing = stochSmooth as StrategyParameterInt;
            ...
            if (context.SharedData.TryGetValue("RsiOversold", out var rsiOs))
                _rsiOversold = rsiOs as StrategyParameterDecimal;
            if (context.SharedData.TryGetValue("RsiOverbought", out var rsiOb))
                _rsiOverbought = rsiOb as StrategyParameterDecimal;
        ...
        public async Task ProcessAsync(Candle candle)
        {
            ...
            // Логирование средней цены безубытка раз в 10 минут в режиме "Подробный"
            // Получаем параметр логирования из SharedData
            if (_context?.SharedData.TryGetValue("LogVerbosity", out var logVerbObj) == true
                && logVerbObj is StrategyParameterString logVerbParam
                && logVerbParam.ValueString == "Подробная")
            {
                // Получаем время последнего лога из SharedData
                DateTime lastLogTime = DateTime.MinValue;
                if (_context.SharedData.TryGetValue("LastAverageBreakevenPriceLogTime", out var lastLogObj)
                    && lastLogObj is DateTime lastLog)
                {
                    lastLogTime = lastLog;
                }
                
                if (DateTime.Now - lastLogTime >= TimeSpan.FromMinutes(10))
                {
                    // Логирование будет выполнено в основном классе бота
                    _context.SharedData["LastAverageBreakevenPriceLogTime"] = DateTime.Now;
                    _context.SharedData["TriggerAverageBreakevenPriceLog"] = true;
                }
            }
            
            // Логирование статуса стопов для всех режимов (с разной частотой и детальностью)
            if (_context?.SharedData.TryGetValue("LogVerbosity", out var logVerbObj2) == true
                && logVerbObj2 is StrategyParameterString logVerbParam2)
            {
                int stopLogIntervalMinutes = logVerbParam2.ValueString == "Подробная"
                    ? 5
                    : (logVerbParam2.ValueString == "Обычная" ? 15 : 30);
                
                // Получаем время последнего лога из SharedData
                DateTime lastStopLogTime = DateTime.MinValue;
                if (_context.SharedData.TryGetValue("LastStopStatusLogTime", out var lastStopLogObj)
                    && lastStopLogObj is DateTime lastStopLog)
                {
                    lastStopLogTime = lastStopLog;
                }
                
                if (DateTime.Now - lastStopLogTime >= TimeSpan.FromMinutes(stopLogIntervalMinutes))
                {
                    // Логирование будет выполнено в основном классе бота
                    _context.SharedData["LastStopStatusLogTime"] = DateTime.Now;
                    _context.SharedData["TriggerStopStatusLog"] = true;
                    _context.SharedData["StopStatusLogPrice"] = candle.Close;
                    _context.SharedData["StopStatusLogVerbosity"] = logVerbParam2.ValueString;
                }
            }
        }
        ...
        private void RefreshParameterCacheIfNeeded()
        {
            ...
            _cachedOpenByRsi = IsParameterOn(_openByRsi);
            _cachedExitByRsi = IsParameterOn(_exitByRsi);
        ...
        private async Task CheckAveraging(Candle candle)
        {
            await Task.Run(() =>
            {
                try
                {
                    ...
                    // ✅ ПРОВЕРКА НЕТОРГОВЫХ ДНЕЙ И ПЕРИОДОВ: если текущее время не разрешено - выходим
                    // Это предотвращает открытие усредняющих позиций в неторговые дни и периоды
                    if (_context.IsTradingTimeAllowed != null && tab != null)
                    {
                        DateTime currentTime = tab.TimeServerCurrent;
                        if (!_context.IsTradingTimeAllowed(currentTime))
                        {
                            return; // Торговля запрещена в это время - не открываем усредняющие позиции
                        }
                    }
        ...
        private async Task CheckExitConditions(Candle candle)
        {
            await Task.Run(() =>
            {
                ...
                {
                    // ✅ ПРОВЕРКА НЕТОРГОВЫХ ДНЕЙ И ПЕРИОДОВ: если текущее время не разрешено - выходим
                    // Это предотвращает закрытие позиций в неторговые дни и периоды
                    if (_context?.IsTradingTimeAllowed != null)
                    {
                        var tab = _context.GetTab();
                        if (tab != null)
                        {
                            ...
                            if (!_context.IsTradingTimeAllowed(currentTime))
                            {
                                return; // Торговля запрещена в это время - не закрываем позиции
                            }
                        }
                    }
                    ...
                    // ✅ RSI — работает независимо от того, готов ли Ишимоку
                    bool rsiReady = dataComponent.TryGetRsiValues(
                        out decimal currentRsi, out decimal previousRsi);
                    ...
                    foreach (var position in activePositions)
                    {
                        ...
                        // ✅ ПРИОРИТЕТ 1: СЛЕДЯЩИЙ СТОП - НАИВЫСШИЙ ПРИОРИТЕТ
                        // Когда следящий стоп включен, он имеет наивысший приоритет перед всеми другими способами закрытия
                        ...
                        if (trailingComponent != null && trailingComponent.IsTrailingEnabled())
                        {
                            ...
                            if (trailingComponent.CheckTrailingStop(position.Number, candle.Close, position))
                            {
                                ...
                                if (trailingLevel > 0 && _context != null)
                                {
                        ...
                        // ✅ ПРИОРИТЕТ 2: ЗАЩИТА ОТ УХОДА ИЗ ПЛЮСА В МИНУС (ТЗ п.4.5)
                        // Проверяем, не вернулась ли позиция в минус после выхода в плюс
                        // Если да - автоматически закрываем по минимальной прибыли
                        if (!exitSignal && CheckLossProtectionExit(position, candle.Close))
                        {
                            exitSignal = true;
                            exitReason = "Loss Protection (went positive, returned to loss)";
                        }

                        // ✅ ПРИОРИТЕТ 3: РУЧНОЙ ТЕЙК-ПРОФИТ
                        // ✅ КРИТИЧНО: Защита от ухода в минус и отката ниже минимальной прибыли
                        // Проверяем, включен ли тейк-профит через SharedData
                        bool isTakeProfitEnabled = false;
                        if (_context?.SharedData.TryGetValue("UseManualTakeProfit", out var tpObj) == true
                            && tpObj is StrategyParameterString tpParam)
                        {
                            isTakeProfitEnabled = tpParam.ValueString == "Включён";
                        }
                        if (!exitSignal && isTakeProfitEnabled)
                        {
                            // ✅ ПРОВЕРКА 1: Защита от ухода из плюса в минус
                            if (CheckLossProtectionExit(position, candle.Close))
                            {
                                exitSignal = true;
                                exitReason = "Loss Protection (went positive, returned to loss)";
                            }
                            // ✅ ПРОВЕРКА 2: Проверка тейк-профита с защитой от отката ниже минимальной прибыли
                            // Получаем параметры тейк-профита из SharedData
                            bool manualTPEnabled = false;
                            decimal manualTPPercent = 0m;
                            if (_context?.SharedData.TryGetValue("UseManualTakeProfit", out var tpEnabledObj) == true
                                && tpEnabledObj is StrategyParameterString tpEnabledParam
                                && tpEnabledParam.ValueString == "Включён"
                                && _context.SharedData.TryGetValue("ManualTakeProfit", out var tpPercentObj) == true
                                && tpPercentObj is StrategyParameterDecimal tpPercentParam)
                            {
                                manualTPEnabled = true;
                                manualTPPercent = tpPercentParam.ValueDecimal;
                            }
                            
                            if (manualTPEnabled)
                            {
                                var riskManagerTP = _cachedRiskManager ?? _context?.GetComponent<RiskManagementComponent>();
                                if (riskManagerTP != null)
                                {
                                    int positionIdTP = position.Number;
                                    decimal entryPriceTP = riskManagerTP.GetEntryPrice(positionIdTP);
                                    if (entryPriceTP > 0)
                                    {
                                        decimal currentPriceTP = candle.Close;
                                        decimal profitPercentTP = position.Direction == Side.Buy
                                            ? (currentPriceTP - entryPriceTP) / entryPriceTP * 100
                                            : (entryPriceTP - currentPriceTP) / entryPriceTP * 100;
                                        
                                        decimal takeProfitPriceTP = position.Direction == Side.Buy
                                            ? entryPriceTP * (1 + manualTPPercent / 100m)
                                            : entryPriceTP * (1 - manualTPPercent / 100m);
                                        
                                        bool priceReachedTakeProfit = position.Direction == Side.Buy
                                            ? candle.High >= takeProfitPriceTP
                                            : candle.Low <= takeProfitPriceTP;
                                        
                                        // Проверяем защиту от убытков
                                        bool wasPositiveTP = riskManagerTP.WentPositive(positionIdTP);
                                        if (wasPositiveTP && profitPercentTP < 0)
                                        {
                                            exitSignal = true;
                                            exitReason = "Loss Protection (went positive, returned to loss)";
                                        }
                                        else if (priceReachedTakeProfit && profitPercentTP > 0)
                                        {
                                            exitSignal = true;
                            ...
                            // Старый вызов метода (закомментирован, так как метод находится в другом классе)
                            // else if (CheckManualTakeProfit(position, candle))
                            {
                                exitSignal = true;
                                exitReason = "TakeProfit";
                            }
                        ...
                        // ✅ ПРИОРИТЕТ 4: СИГНАЛЫ ИНДИКАТОРОВ (Ишимоку)
                        // ✅ КРИТИЧНО: Когда трейлинг и фиксированный TP выключены - закрытие по сигналам индикаторов работает БЕЗ ПРЕПЯТСТВИЙ
                        // ✅ ПОЛНЫЙ ЗАПРЕТ НА ОТРИЦАТЕЛЬНОЕ ЗАКРЫТИЕ НИЖЕ МИНИМАЛЬНОЙ ПРИБЫЛИ:
                        //    - Если позиция в минусе и никогда не была в плюсе → закрытие ЗАБЛОКИРОВАНО
                        //    - Если позиция вернулась в минус после выхода в плюс → закрытие ЗАБЛОКИРОВАНО
                        //    - Если позиция была в плюсе, но прибыль < минимальной → закрытие разрешено ТОЛЬКО по цене минимальной прибыли
                        //    - Если прибыль >= минимальной → закрытие разрешено
                        // Проверяем защиту ПЕРЕД установкой сигнала выхода
                        if (!exitSignal && IsExitSignal(position, tenkanValue, kijunValue))
                        {
                            ...
                            // ✅ ПРОВЕРКА 2: Разрешение на закрытие от RiskManager
                            else
                            {
                                var riskManager = _cachedRiskManager ?? _context?.GetComponent<RiskManagementComponent>();
                                if (riskManager != null && riskManager.CanClosePosition(position.Number, candle.Close, position.Direction == Side.Buy))
                                {
                                    exitSignal = true;
                                ...
                                {
                                    // Закрытие запрещено - логируем и не устанавливаем сигнал
                                    _context?.SendLog(
                                        $"🚫 ВЫХОД ПО СИГНАЛУ ИНДИКАТОРА ЗАБЛОКИРОВАН для позиции #{position.Number}: " +
                                        $"защита от убытков запрещает закрытие (прибыль < минимальной или позиция в минусе)",
                                        LogMessageType.System);
                                }
                            }
                        }
                        ...
                        // ✅ ПРИОРИТЕТ 5: COUNTERINTUITIVE ЛОГИКА ВЫХОДА
                        // ✅ КРИТИЧНО: Когда трейлинг и фиксированный TP выключены - закрытие по counterintuitive работает БЕЗ ПРЕПЯТСТВИЙ
                        // ✅ ПОЛНЫЙ ЗАПРЕТ НА ОТРИЦАТЕЛЬНОЕ ЗАКРЫТИЕ НИЖЕ МИНИМАЛЬНОЙ ПРИБЫЛИ:
                        //    - Если позиция в минусе и никогда не была в плюсе → закрытие ЗАБЛОКИРОВАНО
                        //    - Если позиция вернулась в минус после выхода в плюс → закрытие ЗАБЛОКИРОВАНО
                        //    - Если позиция была в плюсе, но прибыль < минимальной → закрытие разрешено ТОЛЬКО по цене минимальной прибыли
                        ...
                        if (!exitSignal && counterintuitiveExit && useCounterintuitive)
                        {
                            ...
                            decimal ema1 = 0m, ema2 = 0m, ema3 = 0m;
                            if (dataComponent != null && dataComponent.TryGetCounterintuitiveEmaValues(out ema1, out ema2, out ema3))
                            {
                                bool counterintuitiveSignal = false;
                                string counterintuitiveReason = "";
                                
                                // Для LONG: выход когда ema2 < ema1 (разворот тренда)
                                if (position.Direction == Side.Buy && ema2 < ema1)
                                {
                                    counterintuitiveSignal = true;
                                    counterintuitiveReason = "Counterintuitive Exit (LONG)";
                                }
                                // Для SHORT: выход когда ema2 > ema1 (разворот тренда)
                                else if (position.Direction == Side.Sell && ema2 > ema1)
                                {
                                    counterintuitiveSignal = true;
                                    counterintuitiveReason = "Counterintuitive Exit (SHORT)";
                                }
                                
                                // ✅ ПРОВЕРКА ЗАЩИТЫ перед установкой сигнала выхода
                                if (counterintuitiveSignal)
                                {
                                    // ✅ ПРОВЕРКА 1: Защита от ухода из плюса в минус
                                    if (CheckLossProtectionExit(position, candle.Close))
                                    {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            exitSignal = true;
                                            exitReason = counterintuitiveReason;
                                        }
                                        else
                                        {
                                            // Закрытие запрещено - логируем и не устанавливаем сигнал
                                            _context?.SendLog(
                                                $"🚫 ВЫХОД ПО COUNTERINTUITIVE ЗАБЛОКИРОВАН для позиции #{position.Number}: " +
                                                $"защита от убытков запрещает закрытие (прибыль < минимальной или позиция в минусе)",
                                                LogMessageType.System);
                                        }
                                    }
                        ...
                        // ✅ ПРИОРИТЕТ 6: СТОХАСТИК ВЫХОД
                        // ✅ ИСПОЛЬЗУЕМ КЭШИРОВАННЫЙ ПАРАМЕТР
                        // ✅ КРИТИЧНО: Когда трейлинг и фиксированный TP выключены - закрытие по стохастику работает БЕЗ ПРЕПЯТСТВИЙ
                        // ✅ ПОЛНЫЙ ЗАПРЕТ НА ОТРИЦАТЕЛЬНОЕ ЗАКРЫТИЕ НИЖЕ МИНИМАЛЬНОЙ ПРИБЫЛИ:
                        //    - Если позиция в минусе и никогда не была в плюсе → закрытие ЗАБЛОКИРОВАНО
                        //    - Если позиция вернулась в минус после выхода в плюс → закрытие ЗАБЛОКИРОВАНО
                        //    - Если позиция была в плюсе, но прибыль < минимальной → закрытие разрешено ТОЛЬКО по цене минимальной прибыли
                        ...
                        if (!exitSignal && stochReady && exitByStoch)
                        {
                            ...
                            {
                                stochSignal = true;
                                stochReason = "Stochastic Exit (LONG)";
                            }
                            ...
                            {
                                stochSignal = true;
                                stochReason = "Stochastic Exit (SHORT)";
                            }
                            ...
                            if (stochSignal)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        exitSignal = true;
                                        exitReason = stochReason;
                                    }
                                    else
                                    {
                                        // Закрытие запрещено - логируем и не устанавливаем сигнал
                                        _context?.SendLog(
                                            $"🚫 ВЫХОД ПО СТОХАСТИКУ ЗАБЛОКИРОВАН для позиции #{position.Number}: " +
                                            $"защита от убытков запрещает закрытие (прибыль < минимальной или позиция в минусе)",
                                            LogMessageType.System);
                                    }
                                }
                        ...
                        // ✅ ПРИОРИТЕТ 7: RSI ВЫХОД
                        // ✅ ИСПОЛЬЗУЕМ КЭШИРОВАННЫЙ ПАРАМЕТР
                        // ✅ КРИТИЧНО: Когда трейлинг и фиксированный TP выключены - закрытие по RSI работает БЕЗ ПРЕПЯТСТВИЙ
                        // ✅ ПОЛНЫЙ ЗАПРЕТ НА ОТРИЦАТЕЛЬНОЕ ЗАКРЫТИЕ НИЖЕ МИНИМАЛЬНОЙ ПРИБЫЛИ:
                        //    - Если позиция в минусе и никогда не была в плюсе → закрытие ЗАБЛОКИРОВАНО
                        //    - Если позиция вернулась в минус после выхода в плюс → закрытие ЗАБЛОКИРОВАНО
                        //    - Если позиция была в плюсе, но прибыль < минимальной → закрытие разрешено ТОЛЬКО по цене минимальной прибыли
                        //    - Если прибыль >= минимальной → закрытие разрешено
                        bool exitByRsi = _cachedExitByRsi ?? IsParameterOn(_exitByRsi);
                        if (!exitSignal && rsiReady && exitByRsi)
                        {
                            bool rsiSignal = false;
                            string rsiReason = "";
                            
                            // LONG выход: RSI выходит из зоны перекупленности сверху вниз (было > overbought, стало <= overbought)
                            if (position.Direction == Side.Buy &&
                                previousRsi > _rsiOverbought.ValueDecimal && currentRsi <= _rsiOverbought.ValueDecimal)
                            {
                                rsiSignal = true;
                                rsiReason = "RSI Exit (LONG)";
                            }
                            // SHORT выход: RSI выходит из зоны перепроданности снизу вверх (было < oversold, стало >= oversold)
                            else if (position.Direction == Side.Sell &&
                                     previousRsi < _rsiOversold.ValueDecimal && currentRsi >= _rsiOversold.ValueDecimal)
                            {
                                rsiSignal = true;
                                rsiReason = "RSI Exit (SHORT)";
                            }
                            ...
                            if (rsiSignal)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        exitSignal = true;
                                        exitReason = rsiReason;
                                    }
                                    else
                                    {
                                        // Закрытие запрещено - логируем и не устанавливаем сигнал
                                        _context?.SendLog(
                                            $"🚫 ВЫХОД ПО RSI ЗАБЛОКИРОВАН для позиции #{position.Number}: " +
                                            $"защита от убытков запрещает закрытие (прибыль < минимальной или позиция в минусе)",
                                            LogMessageType.System);
                                    }
                                }
                        ...
                        if (exitSignal)
                        {
                            // Закрываем позицию(и) в соответствии с режимом закрытия:
                            //  - "По отдельным сделкам"  => только текущая позиция
                            //  - "Общая позиция"         => вся совокупная позиция по направлению (LONG / SHORT)
                            ClosePositionsAccordingToMode(position, candle.Close, exitReason, activePositions);
                        }
                    }
                }
                catch (Exception ex)
        ...
        /// <summary>
        /// Закрытие позиций в соответствии с выбранным режимом закрытия.
        /// "По отдельным сделкам"  — закрывается только одна позиция.
        /// "Общая позиция"         — закрываются ВСЕ открытые позиции по направлению (LONG или SHORT)
        /// с использованием СРЕДНЕЙ цены входа для расчета минимальной прибыли.
        /// Логика выхода: трейлинг-стоп → фиксированный TP → сигналы индикаторов.
        /// ЖЕСТКИЙ ЗАПРЕТ: нельзя закрывать с прибылью менее (средняя цена входа + минимальная прибыль).
        /// </summary>
        private void ClosePositionsAccordingToMode(
            Position triggerPosition,
            decimal currentPrice,
            string reason,
            List<Position> allActivePositions)
        {
            try
            {
                var riskManager = _cachedRiskManager ?? _context.GetComponent<RiskManagementComponent>();
                ...
                // Если нет RiskManager или менеджера позиций — закрываем только одну позицию по стандартной логике
                if (riskManager == null || positionManager == null)
                {
                    TryClosePosition(triggerPosition, currentPrice, reason);
                    return;
                }

                bool isIndividualMode = riskManager.IsIndividualCloseMode();

                // Режим "По отдельным сделкам" — всё как раньше, только одна позиция
                if (isIndividualMode)
                {
                    TryClosePosition(triggerPosition, currentPrice, reason);
                    return;
                ...
                // ✅ РЕЖИМ "ОБЩАЯ ПОЗИЦИЯ" — работаем с совокупной LONG или SHORT позицией
                Side side = triggerPosition.Direction;
                bool isLong = side == Side.Buy;

                // Берём только открытые позиции по нужному направлению
                var sidePositions = allActivePositions
                    .Where(p => p.State == PositionStateType.Open && p.Direction == side)
                    .ToList();

                if (sidePositions.Count == 0)
                {
                    // На всякий случай, если в момент проверки уже всё закрыто
                    TryClosePosition(triggerPosition, currentPrice, reason);
                    return;
                }

                // ✅ РАСЧЕТ СРЕДНЕЙ ЦЕНЫ ВХОДА по всем позициям одного направления
                decimal totalVolume = 0m;
                decimal totalCost = 0m;
                foreach (var position in sidePositions)
                {
                    decimal entryPrice = riskManager.GetEntryPrice(position.Number);
                    if (entryPrice == 0) entryPrice = position.EntryPrice;
                    
                    decimal volume = position.OpenVolume;
                    totalVolume += volume;
                    totalCost += entryPrice * volume;
                }

                if (totalVolume <= 0)
                {
                    _context.SendLog($"🚫 ОШИБКА: Нулевой объем для расчета средней цены входа", LogMessageType.Error);
                    return;
                }

                decimal averageEntryPrice = totalCost / totalVolume;
                
                // ✅ РАСЧЕТ МИНИМАЛЬНОЙ ПРИБЫЛИ от СРЕДНЕЙ цены входа
                decimal minProfitPercent = 0m;
                if (_context.SharedData.TryGetValue("MinProfitPercent", out var minProfObj)
                    && minProfObj is StrategyParameterDecimal minProfParam)
                {
                ...
                decimal averageMinProfitPrice = isLong
                    ? averageEntryPrice * (1 + minProfitPercent / 100m)
                    : averageEntryPrice * (1 - minProfitPercent / 100m);

                // ✅ ПРОВЕРКА: текущая цена должна быть не ниже средней цены входа + минимальная прибыль
                bool canCloseByAveragePrice = isLong
                    ? currentPrice >= averageMinProfitPrice
                    : currentPrice <= averageMinProfitPrice;

                if (!canCloseByAveragePrice)
                {
                    decimal currentProfitPercent = isLong
                        ? ((currentPrice - averageEntryPrice) / averageEntryPrice) * 100m
                        : ((averageEntryPrice - currentPrice) / averageEntryPrice) * 100m;
                    
                    _context.SendLog(
                        $"🚫 ЗАКРЫТИЕ ОБЩЕЙ {side} ПОЗИЦИИ ЗАПРЕЩЕНО: " +
                        $"Текущая прибыль {currentProfitPercent:F2}% < минимальной {minProfitPercent:F2}% " +
                        $"(Средняя цена входа: {averageEntryPrice:F4}, Текущая: {currentPrice:F4}, Мин.прибыль: {averageMinProfitPrice:F4})",
                        LogMessageType.System);
                    return;
                }

                // ✅ РАСЧЕТ ЦЕНЫ ЗАКРЫТИЯ по логике выхода (трейлинг → TP → сигналы)
                var trailingComponent = _context.GetComponent<TrailingStopComponent>();
                decimal closePrice = CalculateClosePriceForCombinedPosition(
                    sidePositions, 
                    averageEntryPrice, 
                    averageMinProfitPrice, 
                    currentPrice, 
                    isLong, 
                    riskManager, 
                    trailingComponent);

                // ✅ ЗАКРЫТИЕ ВСЕХ ПОЗИЦИЙ по рассчитанной цене
                _context.SendLog(
                    $"🔔 ЗАКРЫТИЕ ОБЩЕЙ {side} ПОЗИЦИИ ({sidePositions.Count} сделок) | " +
                    $"Средняя цена входа: {averageEntryPrice:F4} | " +
                    $"Цена закрытия: {closePrice:F4} | " +
                    $"Причина: {reason}",
                    LogMessageType.System);

                foreach (var position in sidePositions)
                {
                    // ✅ КРИТИЧНО: Используем GetProtectedClosePrice для каждой позиции
                    // Это гарантирует, что даже при использовании средней цены, каждая позиция
                    // не будет закрыта ниже своей индивидуальной минимальной прибыли
                    decimal individualMinProfitPrice = riskManager.GetMinProfitPrice(position.Number);
                    decimal finalPriceForPosition = isLong
                        ? Math.Max(closePrice, individualMinProfitPrice > 0 ? individualMinProfitPrice : averageMinProfitPrice)
                        : Math.Min(closePrice, individualMinProfitPrice > 0 ? individualMinProfitPrice : averageMinProfitPrice);
                    
                    _context.GetTab().CloseAtLimit(position, finalPriceForPosition, position.OpenVolume);
                    
                    // Логирование для каждой позиции
                    decimal entryPrice = riskManager.GetEntryPrice(position.Number);
                    if (entryPrice == 0) entryPrice = position.EntryPrice;
                    decimal profitPercent = isLong
                        ? ((finalPriceForPosition - entryPrice) / entryPrice) * 100m
                        : ((entryPrice - finalPriceForPosition) / entryPrice) * 100m;
                    
                    _context.SendLog(
                        $"   #{position.Number}: Вход {entryPrice:F4} → Выход {finalPriceForPosition:F4} ({profitPercent:F2}%)",
                        LogMessageType.System);
                }
            }
            catch (Exception ex)
            {
                _context.SendLog($"❌ Ошибка массового закрытия по режиму 'Общая позиция': {ex.Message}", LogMessageType.Error);
            }
        }

        /// <summary>
        /// ✅ РАСЧЕТ ЦЕНЫ ЗАКРЫТИЯ для общей позиции по логике выхода:
        /// 1. Если включен трейлинг-стоп → используем его уровень
        /// 2. Если включен фиксированный TP → используем его
        /// 3. Если оба выключены → используем экстремальную цену (для сигналов индикаторов)
        /// ВСЕ варианты с жестким ограничением: не ниже средней цены входа + минимальная прибыль
        /// </summary>
        private decimal CalculateClosePriceForCombinedPosition(
            List<Position> positions,
            decimal averageEntryPrice,
            decimal averageMinProfitPrice,
            decimal currentPrice,
            bool isLong,
            RiskManagementComponent riskManager,
            TrailingStopComponent trailingComponent)
        {
            try
            {
                // ✅ ПРИОРИТЕТ 1: Трейлинг-стоп (если включен и активен)
                bool trailingEnabled = trailingComponent != null && trailingComponent.IsTrailingEnabled();
                if (trailingEnabled)
                {
                    // Проверяем, активен ли трейлинг хотя бы для одной позиции
                    bool anyTrailingActive = positions.Any(p => trailingComponent.IsTrailingActive(p.Number));
                    
                    if (anyTrailingActive)
                    {
                        // Используем лучший (самый выгодный) уровень трейлинга среди всех позиций
                        decimal bestTrailingLevel = 0m;
                        foreach (var position in positions)
                        {
                            decimal trailingLevel = trailingComponent.GetTrailingLevel(position.Number);
                            if (trailingLevel > 0)
                            {
                                if (bestTrailingLevel == 0m)
                                {
                                    bestTrailingLevel = trailingLevel;
                                }
                                else
                                {
                                    // Для LONG: берем максимальный уровень (самый выгодный)
                                    // Для SHORT: берем минимальный уровень (самый выгодный)
                                    if (isLong && trailingLevel > bestTrailingLevel)
                                    {
                                        bestTrailingLevel = trailingLevel;
                                    }
                                    else if (!isLong && trailingLevel < bestTrailingLevel)
                                    {
                                        bestTrailingLevel = trailingLevel;
                                    }
                                }
                        ...
                        if (bestTrailingLevel > 0)
                        {
                            // ✅ ЖЕСТКОЕ ОГРАНИЧЕНИЕ: не ниже средней цены входа + минимальная прибыль
                            decimal finalPrice = isLong
                                ? Math.Max(bestTrailingLevel, averageMinProfitPrice)
                                : Math.Min(bestTrailingLevel, averageMinProfitPrice);
                            
                            _context.SendLog(
                                $"📈 Закрытие по ТРЕЙЛИНГ-СТОПУ: Уровень {bestTrailingLevel:F4} → Финальная {finalPrice:F4} (мин.прибыль: {averageMinProfitPrice:F4})",
                                LogMessageType.System);
                            return finalPrice;
                        }
                    }
                ...
                // ✅ ПРИОРИТЕТ 2: Фиксированный тейк-профит (если включен)
                bool takeProfitOn = false;
                decimal manualTpPercent = 0m;
                if (_context.SharedData.TryGetValue("UseManualTakeProfit", out var useTPObj)
                    && useTPObj is StrategyParameterString useTPParam
                    && useTPParam.ValueString == "Включён")
                {
                    takeProfitOn = true;
                    if (_context.SharedData.TryGetValue("ManualTakeProfit", out var tpPercentObj)
                        && tpPercentObj is StrategyParameterDecimal tpPercentParam)
                    {
                        manualTpPercent = tpPercentParam.ValueDecimal;
                    }
                ...
                if (takeProfitOn && averageEntryPrice > 0 && manualTpPercent > 0)
                {
                    decimal manualTp = isLong
                        ? averageEntryPrice * (1 + manualTpPercent / 100m)
                        : averageEntryPrice * (1 - manualTpPercent / 100m);
                    
                    // ✅ ЖЕСТКОЕ ОГРАНИЧЕНИЕ: не ниже средней цены входа + минимальная прибыль
                    decimal finalPrice = isLong
                        ? Math.Max(manualTp, averageMinProfitPrice)
                        : Math.Min(manualTp, averageMinProfitPrice);
                    
                    _context.SendLog(
                        $"💰 Закрытие по ФИКСИРОВАННОМУ TP: {manualTp:F4} → Финальная {finalPrice:F4} (мин.прибыль: {averageMinProfitPrice:F4})",
                        LogMessageType.System);
                    return finalPrice;
                }

                // ✅ ПРИОРИТЕТ 3: Сигналы индикаторов (используем экстремальную цену)
                // Используем лучшую экстремальную цену среди всех позиций
                decimal bestExtremePrice = 0m;
                if (trailingComponent != null)
                {
                    foreach (var position in positions)
                    {
                        decimal extremePrice = trailingComponent.GetExtremePrice(position.Number, isLong);
                        if (extremePrice > 0)
                        {
                            if (bestExtremePrice == 0m)
                            {
                                bestExtremePrice = extremePrice;
                            }
                            else
                            {
                                // Для LONG: берем максимальную экстремальную цену
                                // Для SHORT: берем минимальную экстремальную цену
                                if (isLong && extremePrice > bestExtremePrice)
                                {
                                    bestExtremePrice = extremePrice;
                                }
                                else if (!isLong && extremePrice < bestExtremePrice)
                                {
                                    bestExtremePrice = extremePrice;
                                }
                            }
                ...
                // Если экстремальная цена не найдена, используем текущую цену
                if (bestExtremePrice == 0m)
                {
                    bestExtremePrice = currentPrice;
                }
                
                // ✅ ЖЕСТКОЕ ОГРАНИЧЕНИЕ: не ниже средней цены входа + минимальная прибыль
                decimal finalPriceSignal = isLong
                    ? Math.Max(bestExtremePrice, averageMinProfitPrice)
                    : Math.Min(bestExtremePrice, averageMinProfitPrice);
                
                _context.SendLog(
                    $"📊 Закрытие по СИГНАЛУ ИНДИКАТОРА: Экстремальная {bestExtremePrice:F4} → Финальная {finalPriceSignal:F4} (мин.прибыль: {averageMinProfitPrice:F4})",
                    LogMessageType.System);
                return finalPriceSignal;
            }
            catch (Exception ex)
            {
                _context.SendLog($"❌ Ошибка расчета цены закрытия для общей позиции: {ex.Message}", LogMessageType.Error);
                // Fallback: используем минимальную прибыль
                return averageMinProfitPrice;
            }
        }
            ...
            decimal senkouA, decimal senkouB, decimal chikou)
        {
            ...
            // ✅ COUNTERINTUITIVE УДАЛЕН ИЗ CheckBuySignals - теперь работает как независимый блок в CheckTradingSignals
            // Логика counterintuitive перенесена в CheckTradingSignals для полной независимости от других типов входов
        ...
        private bool CheckSellSignals(decimal tenkan, decimal kijun, decimal currentPrice = 0m)
        {
            ...
            // ✅ COUNTERINTUITIVE УДАЛЕН ИЗ CheckSellSignals - теперь работает как независимый блок в CheckTradingSignals
            // Логика counterintuitive перенесена в CheckTradingSignals для полной независимости от других типов входов
        ...
        private string GetSellSignalReason(decimal tenkan, decimal kijun, decimal currentPrice)
        {
            return GetSellSignalReason(tenkan, kijun);
        }
        ...
        private void TryOpenLongPosition(Candle candle)
        {
            try
            {
                ...
                // Получаем объем из SharedData
                int volume = 1;
                if (_context.SharedData.TryGetValue("Volume", out var volumeObj) && volumeObj is StrategyParameterInt volumeParam)
                {
                    volume = volumeParam.ValueInt;
                }
                
                // Проверяем возможность открытия (1-я проверка)
                if (!positionManager.CanBotOpenNewPosition(volume, securityKey, currentBar))
                {
                    _context.SendLog($"🚫 CanBotOpenNewPosition запретил открытие LONG (объем: {volume}, бар: {currentBar})", LogMessageType.System);
                    return;
                }
                
                // Проверяем риск-менеджмент
                if (!riskManager.CanOpenPosition(candle.Close, volume, securityKey))
                {
                    _context.SendLog($"🚫 CanOpenPosition запретил открытие LONG (цена: {candle.Close:F4}, объем: {volume})", LogMessageType.System);
                    return;
                }
                
                // Проверяем защиту от дублей
                if (IsDuplicateOrder(Side.Buy, volume, candle.Close, securityKey, currentBar))
                {
                    _context.SendLog($"🚫 IsDuplicateOrder запретил открытие LONG (цена: {candle.Close:F4}, объем: {volume})", LogMessageType.System);
                    return;
                }
                ...
                RememberLastOrder(Side.Buy, volume, candle.Close, securityKey, currentBar);
                
                var longReason = string.IsNullOrWhiteSpace(_pendingOpenReason)
                    ? "Сигнал на LONG (условия индикаторов)"
                    : _pendingOpenReason;
                
                // ✅ ДЕТАЛЬНОЕ ЛОГИРОВАНИЕ ОТКРЫТИЯ: все показатели
                decimal entryPrice = candle.Close;
                decimal minProfitPercent = 0.14m;
                if (_context.SharedData.TryGetValue("MinProfitPercent", out var minProfitParam))
                {
                    var minProfit = minProfitParam as StrategyParameterDecimal;
                    if (minProfit != null)
                        minProfitPercent = minProfit.ValueDecimal;
                }
                decimal minProfitPrice = entryPrice * (1 + minProfitPercent / 100m);
                
                // Получаем номер последней позиции
                int lastPositionNumber = 0;
                var positions = tab?.PositionsOpenAll ?? new List<Position>();
                if (positions.Count > 0)
                {
                    lastPositionNumber = positions.Max(p => p.Number);
                }
                
                _context.SendLog(
                    $"🎯 ОТКРЫТИЕ LONG #{lastPositionNumber} | " +
                    $"Вход: {entryPrice:F4} | Мин.прибыль: {minProfitPrice:F4} ({minProfitPercent:F2}%) | " +
                    $"Объем: {volume} | Сигнал: {longReason}",
                    LogMessageType.Trade);
            }
            catch (Exception ex)
            {
        ...
        private void TryOpenShortPosition(Candle candle)
        {
            try
            {
                ...
                _context.SendLog(
                    $"🎯 ОТКРЫТИЕ SHORT #{lastPositionNumber} | " +
                    $"Вход: {entryPrice:F4} | Мин.прибыль: {minProfitPrice:F4} ({minProfitPercent:F2}%) | " +
                    $"Объем: {_volume.ValueInt} | Сигнал: {shortReason}",
                    LogMessageType.Trade);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// ✅ ЕДИНСТВЕННЫЙ МЕТОД ЗАКРЫТИЯ ПОЗИЦИЙ (РЕЖИМ "ПО ОТДЕЛЬНЫМ СДЕЛКАМ")
        ...
        /// 
        /// ✅ КРИТИЧНО: Каждая позиция закрывается с учетом своей ИНДИВИДУАЛЬНОЙ цены входа и минимальной прибыли:
        /// - Для LONG: абсолютный запрет закрытия ниже (цена входа + минимальная прибыль)
        /// - Для SHORT: абсолютный запрет закрытия выше (цена входа - минимальная прибыль)
        /// 
        /// Логика выхода: трейлинг-стоп → фиксированный TP → сигналы индикаторов
        /// Все варианты с жестким ограничением: не ниже индивидуальной минимальной прибыли для каждой позиции.
        /// </summary>
        private void TryClosePosition(Position position, decimal currentPrice, string reason)
        {
            try
            {
                ...
                // Определяем тип закрытия по цене (перед использованием)
                string closeType = priceSource.Contains("trailing") ? "Трейлинг" 
                    : priceSource.Contains("take-profit") ? "Take-Profit"
                    : priceSource.Contains("min-profit") ? "Мин.прибыль"
                    : "Сигнал";

                // ✅ СОХРАНЕНИЕ ПРИЧИНЫ ЗАКРЫТИЯ для последующего использования в логах и экспорте
                var positionManager = _context.GetComponent<PositionManagerComponent>();
                if (positionManager != null)
                {
                    // Формируем полную причину закрытия: тип закрытия + причина
                    string fullCloseReason = $"{closeType}: {reason}";
                    positionManager.RegisterCloseReason(positionId, fullCloseReason);
                }
                
                // ✅ ЗАКРЫТИЕ ТОЛЬКО ЧЕРЕЗ CloseAtLimit - никаких CloseAtMarket
                // Выбор лучшей доступной цены с жёсткой отсечкой minProfit и учётом лучшей стороны стакана
                decimal bookPrice = 0m;
                try
                {
                    bookPrice = isLong ? _context.GetTab().PriceBestBid : _context.GetTab().PriceBestAsk;
                }
                catch { }
                if (bookPrice <= 0)
                {
                    bookPrice = currentPrice > 0 ? currentPrice : closePrice;
                }

                // ✅ ИТОГОВАЯ ЦЕНА (РЕЖИМ "ПО ОТДЕЛЬНЫМ СДЕЛКАМ"): 
                // Long — не ниже ИНДИВИДУАЛЬНОЙ minProfit (цена входа + минимальная прибыль) и Bid
                // Short — не выше ИНДИВИДУАЛЬНОЙ minProfit (цена входа - минимальная прибыль) и Ask
                // ✅ КРИТИЧНО: Каждая позиция использует свою индивидуальную цену входа и минимальную прибыль
                decimal finalPrice = isLong
                    ? Math.Max(Math.Max(closePrice, bookPrice), minProfitPrice)
                    : Math.Min(Math.Min(closePrice, bookPrice), minProfitPrice);
                
                // ✅ ДОПОЛНИТЕЛЬНАЯ ЗАЩИТА: Используем GetProtectedClosePrice для гарантии минимальной прибыли
                // Это критично для выходов по сигналам индикаторов, когда следящий стоп и тейк-профит выключены
                // ✅ КРИТИЧНО: GetProtectedClosePrice использует ИНДИВИДУАЛЬНУЮ минимальную прибыль для каждой позиции
                finalPrice = riskManager.GetProtectedClosePrice(positionId, finalPrice, isLong);

                // Лог финального выбора ��ровня выхода
                _context.SendLog($"📤 Закрытие #{positionId} [{(isLong ? "Long" : "Short")}] | reason={reason} | target={targetPrice:F4} | book={bookPrice:F4} | minProfit={minProfitPrice:F4} | final={finalPrice:F4}", LogMessageType.System);
                ...
                // ✅ ДЕТАЛЬНОЕ ЛОГИРОВАНИЕ: все показатели доходности, таргеты, история прибыли/убытка
                decimal profitPercent = entryPrice > 0 
                    ? (isLong ? (closePrice - entryPrice) / entryPrice * 100m : (entryPrice - closePrice) / entryPrice * 100m)
                    : 0m;
                decimal profitValue = entryPrice > 0 && position.OpenVolume > 0
                    ? (isLong ? (closePrice - entryPrice) * position.OpenVolume : (entryPrice - closePrice) * position.OpenVolume)
                    : 0m;
                
                // Получаем историю прибыли/убытка из RiskManager
                decimal maxProfitReached = riskManager.GetMaxProfitPercent(positionId);
                bool wasInProfit = riskManager.WasPositionInProfit(positionId);
                
                _context.SendLog(
                    $"🔔 ЗАКРЫТИЕ #{positionId} {position.Direction} | " +
                    $"Вход: {entryPrice:F4} → Выход: {closePrice:F4} ({closeType}) | " +
                    $"PnL: {profitPercent:F2}% ({profitValue:F2}) | " +
                    $"Макс.прибыль: {maxProfitReached:F2}% | " +
                    $"Была в плюсе: {(wasInProfit ? "Да" : "Нет")} | " +
                    $"Мин.прибыль: {minProfitPrice:F4} | " +
                    $"Причина: {reason}",
                    LogMessageType.Trade);
        ...
        /// <summary>
        /// Расчёт ожидаемой цены выхода для компонента стратегии (РЕЖИМ "ПО ОТДЕЛЬНЫМ СДЕЛКАМ")
        ...
        /// 
        /// ✅ КРИТИЧНО: Использует ИНДИВИДУАЛЬНУЮ цену входа и минимальную прибыль для каждой позиции:
        /// - Для LONG: минимальная прибыль = цена входа * (1 + minProfitPercent / 100)
        /// - Для SHORT: минимальная прибыль = цена входа * (1 - minProfitPercent / 100)
        /// 
        /// Приоритет выхода: трейлинг-стоп → фиксированный TP → сигналы индикаторов
        /// Все варианты с жестким ограничением: не ниже индивидуальной минимальной прибыли.
        /// </summary>
            ...
            TrailingStopComponent trailingComponent, decimal entryPrice, decimal currentPrice = 0m)
        {
            ...
            // ✅ КРИТИЧНО: Получаем ЭКСТРЕМАЛЬНУЮ цену (highest для LONG, lowest для SHORT)
            // Это ключевой момент - используем максимум/минимум движения, а не текущую цену
            decimal extremePrice = 0m;
            if (trailingComponent != null)
            {
                extremePrice = trailingComponent.GetExtremePrice(positionId, isLong);
            }
            
            // Если экстремальная цена не найдена, используем текущую цену или цену входа
            if (extremePrice == 0m)
            {
                extremePrice = currentPrice > 0 ? currentPrice : (entryPrice > 0 ? entryPrice : pos.EntryPrice);
            }
            
            // ✅ Для расчета цены закрытия используем ЭКСТРЕМАЛЬНУЮ цену, а не текущую
            // Это гарантирует закрытие по максимуму движения, а не по текущей цене
            decimal priceForClose = extremePrice;
            ...
            if (trailingEnabled)
            {
                ...
                if (trailingActive)
                {
                    ...
                    if (trailingLevel == 0m && entryPrice > 0)
                    {
                        ...
                        // ✅ Используем ЭКСТРЕМАЛЬНУЮ цену для расчета trailingLevel
                        decimal priceForCalculation = extremePrice > 0 ? extremePrice : (currentPrice > 0 ? currentPrice : pos.EntryPrice);
            ...
            // ✅ Приоритет 1: активный трейлинг (используем trailingLevel, но не ниже минимальной прибыли)
            if (trailingActive && trailingLevel > 0)
            {
                // ✅ Используем trailingLevel (который рассчитан от экстремальной цены)
                // но гарантируем, что не ниже минимальной прибыли
                decimal finalPrice = isLong
                ...
                string source = trailingComponent.GetTrailingType() == "ATR"
                    ? "trailing-ATR (extreme-based>=min-profit)"
                    : "trailing (extreme-based>=min-profit)";
                return new ExitInfo { Price = finalPrice, Source = source };
            }

            // ✅ Приоритет 2: ручной тейк‑профит (используем экстремальную цену, но не ниже минимальной прибыли)
            if (takeProfitOn && manualTp > 0)
            {
                // ✅ Используем ЭКСТРЕМАЛЬНУЮ цену, если она достигла тейк-профита
                // или минимальную прибыль, если экстремальная цена еще не достигла TP
                decimal priceToUse = extremePrice > 0 ? extremePrice : (currentPrice > 0 ? currentPrice : entryPrice);
                
                if ((isLong && priceToUse >= manualTp) || (!isLong && priceToUse <= manualTp))
                {
                    decimal finalPrice = isLong
                        ? Math.Max(priceToUse, minProfitPrice)
                        : Math.Min(priceToUse, minProfitPrice);
                    
                    string source = finalPrice == minProfitPrice
                        ? "take-profit==min-profit"
                        : "take-profit (extreme-based>=min-profit)";
                    return new ExitInfo { Price = finalPrice, Source = source };
                }
                
                // Если экстремальная цена еще не достигла TP, используем минимальную прибыль
                if (minProfitPrice > 0)
                {
                    decimal finalPrice = isLong
            ...
            // ✅ Приоритет 3: ЭКСТРЕМАЛЬНАЯ цена или минимальная прибыль (для выходов по сигналам индикаторов)
            // Это критично - при выходе по обратным сигналам используем ЭКСТРЕМАЛЬНУЮ цену, а не текущую
            if (minProfitPrice > 0)
            {
                // ✅ Используем ЭКСТРЕМАЛЬНУЮ цену для закрытия по сигналам индикаторов
                // Это гарантирует закрытие по максимуму движения, а не по текущей цене
                decimal priceToUse = extremePrice > 0 ? extremePrice : (currentPrice > 0 ? currentPrice : entryPrice);
                decimal finalPrice = isLong
                    ? Math.Max(priceToUse, minProfitPrice)
                ...
                string source = extremePrice > 0 
                    ? "signal-exit (extreme-price>=min-profit)"
                    : "min-profit (current>=min-profit)";
                return new ExitInfo { Price = finalPrice, Source = source };
            }

            // Fallback: используем экстремальную цену или цену входа
            decimal fallbackPrice = extremePrice > 0 ? extremePrice : (currentPrice > 0 ? currentPrice : entryPrice);
            return new ExitInfo { Price = fallbackPrice, Source = extremePrice > 0 ? "extreme-price" : "entry" };
        }
        
        private bool CheckManualTakeProfit(Position position, Candle candle)
        {
            // Проверяем, включен ли тейк-профит через SharedData
            bool isTakeProfitEnabled = false;
            decimal manualTakeProfitPercent = 0m;
            if (_context?.SharedData.TryGetValue("UseManualTakeProfit", out var tpEnabledObj) == true
                && tpEnabledObj is StrategyParameterString tpEnabledParam
                && tpEnabledParam.ValueString == "Включён"
                && _context.SharedData.TryGetValue("ManualTakeProfit", out var tpPercentObj) == true
                && tpPercentObj is StrategyParameterDecimal tpPercentParam)
            {
                ...
                manualTakeProfitPercent = tpPercentParam.ValueDecimal;
            }
            ...
            try
            {
                ...
                // ✅ КРИТИЧНО: ЗАЩИТА ОТ УХОДА ИЗ ПЛЮСА В МИНУС И ОТКАТА НИЖЕ МИНИМАЛЬНОЙ ПРИБЫЛИ (ТЗ п.4.5)
                // Проверяем, была ли позиция в плюсе, но вернулась в минус или откатилась ниже минимальной прибыли
                // Если да - закрываем по минимальной прибыли, а не по тейк-профиту
                bool wasPositive = riskManager.WentPositive(positionId);
                if (wasPositive)
                {
                    // Получаем минимальную прибыль из SharedData
                    decimal minProfitPercent = 0.14m;
                    if (_context?.SharedData.TryGetValue("MinProfitPercent", out var minProfObj) == true
                        && minProfObj is StrategyParameterDecimal minProfParam)
                    {
                        minProfitPercent = minProfParam.ValueDecimal;
                    }

                    // ✅ ЭТАЛОН: Если позиция была в плюсе, но вернулась в минус - закрываем по минимальной прибыли
                    if (profitPercent < 0)
                    {
                        _context?.SendLog(
                            $"🛡️ ЗАЩИТА ОТ УХОДА В МИНУС (ручной TP): Позиция #{positionId} была в плюсе, но вернулась в {profitPercent:F2}%. " +
                            $"Закрываем по минимальной прибыли вместо тейк-профита.",
                            LogMessageType.System);
                        return true;
                    }
                    
                    // ✅ ЭТАЛОН: Если позиция была в плюсе, но откатилась ниже минимальной прибыли (не достигнув тейк-профита) - закрываем по минимальной прибыли
                    if (profitPercent < minProfitPercent)
                    {
                        // Проверяем, что тейк-профит еще не достигнут
                        if (profitPercent < manualTakeProfitPercent)
                        {
                            _context?.SendLog(
                                $"🛡️ ЗАЩИТА ОТ ОТКАТА НИЖЕ МИНИМАЛЬНОЙ ПРИБЫЛИ (ручной TP): Позиция #{positionId} была в плюсе, но откатилась до {profitPercent:F2}% " +
                                $"(минимальная прибыль: {minProfitPercent:F2}%, тейк-профит: {manualTakeProfitPercent:F2}%). " +
                                $"Закрываем по минимальной прибыли, не допуская ухода ниже уровня минимальной прибыли.",
                                LogMessageType.System);
                            return true;
                        }
                    }
                ...
                // ✅ ИСПРАВЛЕНИЕ ДЛЯ РУЧНОГО ФИКС ПРОФИТА: Срабатывание при достижении уровня
                // Тейк-профит срабатывает если цена достигла уровня тейк-профита в течение свечи
                decimal takeProfitPrice = position.Direction == Side.Buy
                    ? entryPrice * (1 + manualTakeProfitPercent / 100m)
                    : entryPrice * (1 - manualTakeProfitPercent / 100m);
                ...
                // ✅ НОВАЯ ЛОГИКА (ПРАВИЛЬНАЯ): Если позиция достигла тейк-профита И позиция в плюсе - закрываем
                if (priceReachedTakeProfit && profitPercent > 0)
                {
                    _context?.SendLog(
                        $"💰 ТЕЙК-ПРОФИТ ДОСТИГНУТ для позиции #{positionId}: Прибыль {profitPercent:F2}% | " +
                        $"Цена входа: {entryPrice:F4} | Уровень TP: {takeProfitPrice:F4} | Текущая цена: {currentPrice:F4}",
                        LogMessageType.System);
                    return true; // Закрываем независимо от минимальной прибыли
                }

                // ✅ ЗАЩИТА ОТ УБЫТКОВ: Если позиция достигла тейк-профита, но позиция в минусе - не закрываем
                if (priceReachedTakeProfit && profitPercent <= 0)
                {
                    _context?.SendLog(
                        $"🚫 ТЕЙК-ПРОФИТ ДОСТИГНУТ, НО ПОЗИЦИЯ В МИНУСЕ для позиции #{positionId}: Прибыль {profitPercent:F2}%. " +
                        $"Закрытие заблокировано защитой от убытков.",
                        LogMessageType.System);
                    return false; // Защита от убытков
                }
        ...
        /// <summary>
        /// ✅ ЗАЩИТА ОТ УХОДА ИЗ ПЛЮСА В МИНУС (ТЗ п.4.5)
        /// Проверяет, не вернулась ли позиция в минус после выхода в плюс.
        /// Если позиция была в плюсе, но вернулась в минус - возвращает true для закрытия по минимальной прибыли.
        /// </summary>
        private bool CheckLossProtectionExit(Position position, decimal currentPrice)
        {
            try
            {
                var riskManager = _context.GetComponent<RiskManagementComponent>();
                ...
                // Проверяем, была ли позиция в плюсе
                bool wasPositive = riskManager.WentPositive(positionId);
                if (!wasPositive) return false; // Позиция никогда не была в плюсе - не закрываем

                // Рассчитываем текущую прибыль
                decimal currentProfitPercent = position.Direction == Side.Buy
                    ? (currentPrice - entryPrice) / entryPrice * 100m
                    : (entryPrice - currentPrice) / entryPrice * 100m;

                // Получаем минимальную прибыль
                decimal minProfitPercent = 0m;
                if (_context.SharedData.TryGetValue("MinProfitPercent", out var minProfObj)
                    && minProfObj is StrategyParameterDecimal minProfParam)
                {
                ...
                // ✅ КРИТЕРИЙ: Позиция была в плюсе, но вернулась в минус (текущая прибыль < 0)
                // ИЛИ текущая прибыль < минимальной прибыли
                if (currentProfitPercent < 0 || currentProfitPercent < minProfitPercent)
                {
                    _context.SendLog(
                        $"🛡️ ЗАЩИТА ОТ УХОДА В МИНУС: Позиция #{positionId} была в плюсе, но вернулась в {currentProfitPercent:F2}% " +
                        $"(минимальная прибыль: {minProfitPercent:F2}%). Закрываем по минимальной прибыли.",
                        LogMessageType.System);
                    return true;
                }
            ...
            {
                _context.SendLog($"❌ Ошибка в CheckLossProtectionExit: {ex.Message}", LogMessageType.Error);
                return false;
            }
        }
    ...
    public class IshimokuAdaptiveTrailing : BotPanel
    {
        ...
        // ✅ ПАРАМЕТРЫ САМООБУЧАЕМОГО СЛЕДЯЩЕГО СТОПА
        private StrategyParameterDecimal _selfLearningBaseDistance;
        private StrategyParameterDecimal _selfLearningHighProfitMultiplier;
        private StrategyParameterDecimal _selfLearningLowProfitMultiplier;
        private StrategyParameterDecimal _selfLearningHighVolatilityMultiplier;
        private StrategyParameterDecimal _selfLearningLowVolatilityMultiplier;
        private StrategyParameterDecimal _selfLearningMinDistance;
        private StrategyParameterDecimal _selfLearningMaxDistance;
        private StrategyParameterInt _selfLearningHistorySize;
        private StrategyParameterDecimal _selfLearningProfitThresholdHigh;
        private StrategyParameterDecimal _selfLearningProfitThresholdLow;
        private StrategyParameterDecimal _selfLearningVolatilityThresholdHigh;
        private StrategyParameterDecimal _selfLearningVolatilityThresholdLow;
        private StrategyParameterString _useManualTakeProfit;
        private StrategyParameterDecimal _manualTakeProfit;
        private StrategyParameterDecimal _minProfitPercentParam;
        private StrategyParameterInt _maxOpenPositions;
        ...
        // Время последнего лога средней цены безубытка (раз в 10 минут в режиме "Подробный")
        private DateTime _lastAverageBreakevenPriceLogTime = DateTime.MinValue;
        // Время последнего лога статуса стопов (раз в 5 минут в режиме "Подробный")
        private DateTime _lastStopStatusLogTime = DateTime.MinValue;
        ...
        private void CreateParameters()
        {
            ...
            // ✅ ПАРАМЕТРЫ RSI
            _rsiPeriod = CreateParameter("RSI период", 14, 5, 50, 1, ParameterGroups.Rsi);
            _rsiOversold = CreateParameter("RSI перепроданность", 30.0m, 10.0m, 50.0m, 1.0m, ParameterGroups.Rsi);
            _rsiOverbought = CreateParameter("RSI перекупленность", 70.0m, 50.0m, 90.0m, 1.0m, ParameterGroups.Rsi);
            ...
            _openByRsi = CreateParameter("Открытие: RSI", "Выключено", new[] { "Включено", "Выключено" }, ParameterGroups.Rsi);
            ...
            _exitByRsi = CreateParameter("Выход: RSI", "Выключено", new[] { "Включено", "Выключено" }, ParameterGroups.Rsi);
            
            // ✅ СЛЕДЯЩИЙ СТОП В ОТДЕЛЬНОЙ ВКЛАДКЕ
            _useTrailingStop = CreateParameter(
                "Включить следящий стоп — главный переключатель трейлинга (ВКЛ: используется выбранный тип, ВЫКЛ: трейлинг полностью отключён)",
                "Выключен",
                new[] { "Включён", "Выключен" },
                ParameterGroups.TrailingStop);

            _trailingType = CreateParameter(
                "Тип трейлинга — Фиксированный: % от экстремума; ATR: волатильностный стоп; Самообучаемый: динамический стоп по прибыли и волатильности",
                "Фиксированный",
                new[] { "Фиксированный", "ATR", "Самообучаемый" },
                ParameterGroups.TrailingStop);

            _trailingStartPercent = CreateParameter(
                "Старт трейлинга % — при какой накопленной прибыли включать следящий стоп (меньше = раньше включается, больше = позже даёт цене 'подышать'). Диапазон: 0.10–10.00 %",
                0.15m, 0.1m, 10.0m, 0.01m, ParameterGroups.TrailingStop);

_trailingDistancePercent = CreateParameter(
    "Дистанция трейлинга % — насколько близко стоп 'прилипает' к экстремуму (меньше = плотнее, чаще выбивает; больше = свободнее, больше отдаём откатам). Диапазон: 0.01–10.00 %",
    0.01m, 0.01m, 10.0m, 0.01m, ParameterGroups.TrailingStop);
            
            // ATR параметры
            _atrPeriod = CreateParameter(
                "ATR период — длина окна для расчёта волатильности (меньше = чувствительнее к шуму, больше = более сглаженный ATR). Диапазон: 5–50",
                8, 5, 50, 1, ParameterGroups.TrailingStop);

            _atrMultiplier = CreateParameter(
                "ATR множитель — во сколько ATR отстоит стоп от цены (меньше = агрессивнее, ближе к цене; больше = консервативнее, дальше от цены). Диапазон: 0.50–5.00",
                1.2m, 0.5m, 5.0m, 0.1m, ParameterGroups.TrailingStop);
            
            // ✅ ПАРАМЕТРЫ САМООБУЧАЕМОГО СЛЕДЯЩЕГО СТОПА
            _selfLearningBaseDistance = CreateParameter(
                "Самообучаемый: Базовая дистанция % — стартовая дистанция до стопа до адаптации (меньше = ближе к цене, больше = дальше). Диапазон: 0.05–5.00 %",
                0.1m, 0.05m, 5.0m, 0.01m, ParameterGroups.TrailingStop);

            _selfLearningHighProfitMultiplier = CreateParameter(
                "Самообучаемый: Множитель при высокой прибыли — насколько увеличивать дистанцию, если прибыль значительно выше средней (больше = даём больше 'дышать' тренду). Диапазон: 0.50–3.00",
                1.3m, 0.5m, 3.0m, 0.1m, ParameterGroups.TrailingStop);

            _selfLearningLowProfitMultiplier = CreateParameter(
                "Самообучаемый: Множитель при низкой прибыли — насколько сжимать дистанцию, если прибыль ниже средней (меньше 1.0 = жёстче, ближе к цене). Диапазон: 0.30–1.50",
                0.8m, 0.3m, 1.5m, 0.1m, ParameterGroups.TrailingStop);

            _selfLearningHighVolatilityMultiplier = CreateParameter(
                "Самообучаемый: Множитель при высокой волатильности — как расширять дистанцию в турбулентном рынке (больше = дальше от цены, меньше шума). Диапазон: 0.50–3.00",
                1.2m, 0.5m, 3.0m, 0.1m, ParameterGroups.TrailingStop);

            _selfLearningLowVolatilityMultiplier = CreateParameter(
                "Самообучаемый: Множитель при низкой волатильности — как сжимать дистанцию на спокойном рынке (меньше = ближе к цене, больше захват мелких колебаний). Диапазон: 0.30–1.50",
                0.9m, 0.3m, 1.5m, 0.1m, ParameterGroups.TrailingStop);

            _selfLearningMinDistance = CreateParameter(
                "Самообучаемый: Минимальная дистанция % — нижняя граница, ближе которой стоп никогда не подходит к цене (защита от чрезмерно жёсткого стопа). Диапазон: 0.01–1.00 %",
                0.05m, 0.01m, 1.0m, 0.01m, ParameterGroups.TrailingStop);

            _selfLearningMaxDistance = CreateParameter(
                "Самообучаемый: Максимальная дистанция % — верхняя граница, дальше которой стоп никогда не уходит (защита от слишком 'ленивого' стопа). Диапазон: 1.00–10.00 %",
                5.0m, 1.0m, 10.0m, 0.1m, ParameterGroups.TrailingStop);

            _selfLearningHistorySize = CreateParameter(
                "Самообучаемый: Размер истории — сколько последних наблюдений прибыли/волатильности учитывать (меньше = быстрее реагирует, больше = стабильнее). Диапазон: 10–200",
                50, 10, 200, 5, ParameterGroups.TrailingStop);

            _selfLearningProfitThresholdHigh = CreateParameter(
                "Самообучаемый: Порог высокой прибыли (x среднего) — выше какого множителя средней прибыли считать прибыль 'высокой' (включает растяжку дистанции). Диапазон: 1.0–2.0",
                1.2m, 1.0m, 2.0m, 0.1m, ParameterGroups.TrailingStop);

            _selfLearningProfitThresholdLow = CreateParameter(
                "Самообучаемый: Порог низкой прибыли (x среднего) — ниже какого множителя средней прибыли считать прибыль 'низкой' (включает сжатие дистанции). Диапазон: 0.5–1.0",
                0.8m, 0.5m, 1.0m, 0.1m, ParameterGroups.TrailingStop);

            _selfLearningVolatilityThresholdHigh = CreateParameter(
                "Самообучаемый: Порог высокой волатильности (x среднего) — при каком уровне волатильности считать рынок 'турбулентным' (расширять дистанцию). Диапазон: 1.0–3.0",
                1.5m, 1.0m, 3.0m, 0.1m, ParameterGroups.TrailingStop);

            _selfLearningVolatilityThresholdLow = CreateParameter(
                "Самообучаемый: Порог низкой волатильности (x среднего) — при каком уровне волатильности считать рынок 'спокойным' (сжимать дистанцию). Диапазон: 0.1–1.0",
                0.5m, 0.1m, 1.0m, 0.1m, ParameterGroups.TrailingStop);
            ...
            _psoSwarmSize = CreateParameter("PSO: Размер роя (50-10000) ⚡ Больше=шире поиск, турбинный режим. ↘ Меньше=быстрее, риск локального оптимума", 
                100, 50, 10000, 10, "AI Оптимизация");
            _psoMaxIterations = CreateParameter("PSO: Макс. итераций (100-10000) ⚡ Циклы поиска. ↗ Больше=точнее параметры. ↘ Меньше=быстрее завершение", 
                1000, 100, 10000, 50, "AI Оптимизация");
            ...
            _optimizeTenkanLength = CreateParameter("Оптимизировать: Tenkan период — основной быстрый период Ишимоку (влияет на чувствительность входов)", true, "Выбор параметров оптимизации");
            _optimizeKijunLength = CreateParameter("Оптимизировать: Kijun период — основной медленный период Ишимоку (влияет на фильтрацию тренда)", true, "Выбор параметров оптимизации");
            _optimizeSenkouBLength = CreateParameter("Оптимизировать: Senkou span B период — глубина облака Ишимоку (влияет на ширину зон поддержки/сопротивления)", true, "Выбор параметров оптимизации");
            _optimizeSenkouOffset = CreateParameter("Оптимизировать: Senkou смещение — сдвиг облака вперёд по времени (меняет заблаговременность сигналов)", true, "Выбор параметров оптимизации");
            _optimizeStochPeriod = CreateParameter("Оптимизировать: Stochastic %K период — длина окна осциллятора (чувствительность к колебаниям)", true, "Выбор параметров оптимизации");
            _optimizeStochSmoothing = CreateParameter("Оптимизировать: Stochastic сглаживание %K — сглаживание основной линии стохастика (меньше = резче, больше = плавнее)", true, "Выбор параметров оптимизации");
            _optimizeStochDPeriod = CreateParameter("Оптимизировать: Stochastic %D период — период сигнальной линии (влияет на запаздывание входов/выходов)", true, "Выбор параметров оптимизации");
            _optimizeStochOversold = CreateParameter("Оптимизировать: Stochastic перепроданность % — уровень для поиска разворотов снизу (чем ниже, тем реже, но сильнее сигналы)", true, "Выбор параметров оптимизации");
            _optimizeStochOverbought = CreateParameter("Оптимизировать: Stochastic перекупленность % — уровень для поиска разворотов сверху (чем выше, тем реже, но сильнее сигналы)", true, "Выбор параметров оптимизации");
            
            // ✅ Параметры оптимизации RSI
            _optimizeRsiPeriod = CreateParameter("Оптимизировать: RSI период — длина окна RSI (короче = резче, длиннее = плавнее)", true, "Выбор параметров оптимизации");
            _optimizeRsiOversold = CreateParameter("Оптимизировать: RSI перепроданность — уровень для покупок от перепроданности (ниже = реже, но сильнее сигналы)", true, "Выбор параметров оптимизации");
            _optimizeRsiOverbought = CreateParameter("Оптимизировать: RSI перекупленность — уровень для продаж от перекупленности (выше = реже, но сильнее сигналы)", true, "Выбор параметров оптимизации");
            
            _optimizeAveragingLevel1 = CreateParameter("Оптимизировать: Уровень усреднения 1 (%) — на каком откате добавлять первую доливку", true, "Выбор параметров оптимизации");
            _optimizeAveragingLevel2 = CreateParameter("Оптимизировать: Уровень усреднения 2 (%) — вторая ступень усреднения", true, "Выбор параметров оптимизации");
            _optimizeAveragingLevel3 = CreateParameter("Оптимизировать: Уровень усреднения 3 (%) — третья ступень усреднения", true, "Выбор параметров оптимизации");
            _optimizeAveragingLevel4 = CreateParameter("Оптимизировать: Уровень усреднения 4 (%) — четвёртая ступень усреднения", true, "Выбор параметров оптимизации");
            _optimizeAveragingLevel5 = CreateParameter("Оптимизировать: Уровень усреднения 5 (%) — пятая ступень усреднения", true, "Выбор параметров оптимизации");
            _optimizeAveragingLevel6 = CreateParameter("Оптимизировать: Уровень усреднения 6 (%) — шестая ступень усреднения", true, "Выбор параметров оптимизации");
            _optimizeAveragingLevel7 = CreateParameter("Оптимизировать: Уровень усреднения 7 (%) — седьмая ступень усреднения", true, "Выбор параметров оптимизации");
            _optimizeAveragingLevel8 = CreateParameter("Оптимизировать: Уровень усреднения 8 (%) — восьмая ступень усреднения", true, "Выбор параметров оптимизации");
            _optimizeAveragingLevel9 = CreateParameter("Оптимизировать: Уровень усреднения 9 (%) — девятая ступень усреднения", true, "Выбор параметров оптимизации");
            _optimizeAveragingLevel10 = CreateParameter("Оптимизировать: Уровень усреднения 10 (%) — десятая ступень усреднения", true, "Выбор параметров оптимизации");
            _optimizeAveragingLevel11 = CreateParameter("Оптимизировать: Уровень усреднения 11 (%) — одиннадцатая ступень усреднения", true, "Выбор параметров оптимизации");
            _optimizeAveragingLevel12 = CreateParameter("Оптимизировать: Уровень усреднения 12 (%) — двенадцатая ступень усреднения", true, "Выбор параметров оптимизации");

            _optimizeMinProfitPercent = CreateParameter("Оптимизировать: Минимальная прибыль % — жёсткий порог безубытка (включая комиссии)", true, "Выбор параметров оптимизации");

            // ✅ 16 ПАРАМЕТРОВ СЛЕДЯЩЕГО СТОПА — АННОТАЦИИ СОВПАДАЮТ СО ВКЛАДКОЙ \"СЛЕДЯЩИЙ СТОП\"
            _optimizeTrailingStartPercent = CreateParameter(
                "Оптимизировать: Старт трейлинга % — при какой накопленной прибыли включать следящий стоп (меньше = раньше включается, больше = позже даёт цене 'подышать'). Диапазон: 0.10–10.00 %",
                true, "Выбор параметров оптимизации");

            _optimizeTrailingDistancePercent = CreateParameter(
                "Оптимизировать: Дистанция трейлинга % — насколько близко стоп 'прилипает' к экстремуму (меньше = плотнее, чаще выбивает; больше = свободнее, больше отдаём откатам). Диапазон: 0.01–5.00 %",
                true, "Выбор параметров оптимизации");

            _optimizeSelfLearningTrailing = CreateParameter(
                "Оптимизировать: Включение самообучаемого трейлинга — разрешить AI подбирать параметры адаптивного стопа",
                false, "Выбор параметров оптимизации");

            _optimizeManualTakeProfit = CreateParameter(
                "Оптимизировать: Ручной тейк-профит % — цель фиксированной прибыли для принудительного выхода",
                true, "Выбор параметров оптимизации");

            _optimizeBreakEvenTriggerPercent = CreateParameter(
                "Оптимизировать: Триггер безубытка % — при какой прибыли переводить стоп в безубыток",
                true, "Выбор параметров оптимизации");

            _optimizeMaxSpreadPercent = CreateParameter(
                "Оптимизировать: Макс. спред % — максимальный допустимый спред для открытия/закрытия сделок",
                true, "Выбор параметров оптимизации");

            _optimizeATRPeriod = CreateParameter(
                "Оптимизировать: ATR период — длина окна для расчёта волатильности (меньше = чувствительнее, больше = сглаженнее). Диапазон: 5–50",
                true, "Выбор параметров оптимизации");

            _optimizeATRMultiplier = CreateParameter(
                "Оптимизировать: ATR множитель — во сколько ATR отстоит стоп от цены (меньше = ближе, больше = дальше). Диапазон: 0.50–5.00",
                true, "Выбор параметров оптимизации");
            
            // ✅ Параметры оптимизации самообучаемого трейлинга — АННОТАЦИИ ИДЕНТИЧНЫ ВКЛАДКЕ \"СЛЕДЯЩИЙ СТОП\"
            _optimizeSelfLearningBaseDistance = CreateParameter(
                "Оптимизировать: Самообучаемый: Базовая дистанция % — стартовая дистанция до стопа до адаптации (меньше = ближе к цене, больше = дальше). Диапазон: 0.05–5.00 %",
                false, "Выбор параметров оптимизации");

            _optimizeSelfLearningHighProfitMultiplier = CreateParameter(
                "Оптимизировать: Самообучаемый: Множитель при высокой прибыли — насколько увеличивать дистанцию, если прибыль значительно выше средней (больше = даём больше 'дышать' тренду). Диапазон: 0.50–3.00",
                false, "Выбор параметров оптимизации");

            _optimizeSelfLearningLowProfitMultiplier = CreateParameter(
                "Оптимизировать: Самообучаемый: Множитель при низкой прибыли — насколько сжимать дистанцию, если прибыль ниже средней (меньше 1.0 = жёстче, ближе к цене). Диапазон: 0.30–1.50",
                false, "Выбор параметров оптимизации");

            _optimizeSelfLearningHighVolatilityMultiplier = CreateParameter(
                "Оптимизировать: Самообучаемый: Множитель при высокой волатильности — как расширять дистанцию в турбулентном рынке (больше = дальше от цены, меньше шума). Диапазон: 0.50–3.00",
                false, "Выбор параметров оптимизации");

            _optimizeSelfLearningLowVolatilityMultiplier = CreateParameter(
                "Оптимизировать: Самообучаемый: Множитель при низкой волатильности — как сжимать дистанцию на спокойном рынке (меньше = ближе к цене, больше захват мелких колебаний). Диапазон: 0.30–1.50",
                false, "Выбор параметров оптимизации");

            _optimizeSelfLearningMinDistance = CreateParameter(
                "Оптимизировать: Самообучаемый: Минимальная дистанция % — нижняя граница, ближе которой стоп никогда не подходит к цене (защита от чрезмерно жёсткого стопа). Диапазон: 0.01–1.00 %",
                false, "Выбор параметров оптимизации");

            _optimizeSelfLearningMaxDistance = CreateParameter(
                "Оптимизировать: Самообучаемый: Максимальная дистанция % — верхняя граница, дальше которой стоп никогда не уходит (защита от слишком 'ленивого' стопа). Диапазон: 1.00–10.00 %",
                false, "Выбор параметров оптимизации");

            _optimizeSelfLearningHistorySize = CreateParameter(
                "Оптимизировать: Самообучаемый: Размер истории — сколько последних наблюдений прибыли/волатильности учитывать (меньше = быстрее реагирует, больше = стабильнее). Диапазон: 10–200",
                false, "Выбор параметров оптимизации");

            _optimizeSelfLearningProfitThresholdHigh = CreateParameter(
                "Оптимизировать: Самообучаемый: Порог высокой прибыли (x среднего) — выше какого множителя средней прибыли считать прибыль 'высокой' (включает растяжку дистанции). Диапазон: 1.0–2.0",
                false, "Выбор параметров оптимизации");

            _optimizeSelfLearningProfitThresholdLow = CreateParameter(
                "Оптимизировать: Самообучаемый: Порог низкой прибыли (x среднего) — ниже какого множителя средней прибыли считать прибыль 'низкой' (включает сжатие дистанции). Диапазон: 0.5–1.0",
                false, "Выбор параметров оптимизации");

            _optimizeSelfLearningVolatilityThresholdHigh = CreateParameter(
                "Оптимизировать: Самообучаемый: Порог высокой волатильности (x среднего) — при каком уровне волатильности считать рынок 'турбулентным' (расширять дистанцию). Диапазон: 1.0–3.0",
                false, "Выбор параметров оптимизации");

            _optimizeSelfLearningVolatilityThresholdLow = CreateParameter(
                "Оптимизировать: Самообучаемый: Порог низкой волатильности (x среднего) — при каком уровне волатильности считать рынок 'спокойным' (сжимать дистанцию). Диапазон: 0.1–1.0",
                false, "Выбор параметров оптимизации");
        ...
        private void SaveParametersToContext()
        {
            ...
            _componentContext.SharedData[SharedDataKeys.RsiPeriod] = _rsiPeriod;
            _componentContext.SharedData[SharedDataKeys.RsiOversold] = _rsiOversold;
            _componentContext.SharedData[SharedDataKeys.RsiOverbought] = _rsiOverbought;
            ...
            _componentContext.SharedData[SharedDataKeys.OpenByRsi] = _openByRsi;
            ...
            _componentContext.SharedData[SharedDataKeys.ExitByRsi] = _exitByRsi;
            ...
            _componentContext.SharedData[SharedDataKeys.OptimizeRsiPeriod] = _optimizeRsiPeriod;
            _componentContext.SharedData[SharedDataKeys.OptimizeRsiOversold] = _optimizeRsiOversold;
            _componentContext.SharedData[SharedDataKeys.OptimizeRsiOverbought] = _optimizeRsiOverbought;
            
            // ✅ Параметры самообучаемого следящего стопа
            _componentContext.SharedData[SharedDataKeys.SelfLearningBaseDistance] = _selfLearningBaseDistance;
            _componentContext.SharedData[SharedDataKeys.SelfLearningHighProfitMultiplier] = _selfLearningHighProfitMultiplier;
            _componentContext.SharedData[SharedDataKeys.SelfLearningLowProfitMultiplier] = _selfLearningLowProfitMultiplier;
            _componentContext.SharedData[SharedDataKeys.SelfLearningHighVolatilityMultiplier] = _selfLearningHighVolatilityMultiplier;
            _componentContext.SharedData[SharedDataKeys.SelfLearningLowVolatilityMultiplier] = _selfLearningLowVolatilityMultiplier;
            _componentContext.SharedData[SharedDataKeys.SelfLearningMinDistance] = _selfLearningMinDistance;
            _componentContext.SharedData[SharedDataKeys.SelfLearningMaxDistance] = _selfLearningMaxDistance;
            _componentContext.SharedData[SharedDataKeys.SelfLearningHistorySize] = _selfLearningHistorySize;
            _componentContext.SharedData[SharedDataKeys.SelfLearningProfitThresholdHigh] = _selfLearningProfitThresholdHigh;
            _componentContext.SharedData[SharedDataKeys.SelfLearningProfitThresholdLow] = _selfLearningProfitThresholdLow;
            _componentContext.SharedData[SharedDataKeys.SelfLearningVolatilityThresholdHigh] = _selfLearningVolatilityThresholdHigh;
            _componentContext.SharedData[SharedDataKeys.SelfLearningVolatilityThresholdLow] = _selfLearningVolatilityThresholdLow;
            ...
            // ✅ Флаги оптимизации параметров самообучаемого трейлинга
            _componentContext.SharedData[SharedDataKeys.OptimizeSelfLearningBaseDistance] = _optimizeSelfLearningBaseDistance;
            _componentContext.SharedData[SharedDataKeys.OptimizeSelfLearningHighProfitMultiplier] = _optimizeSelfLearningHighProfitMultiplier;
            _componentContext.SharedData[SharedDataKeys.OptimizeSelfLearningLowProfitMultiplier] = _optimizeSelfLearningLowProfitMultiplier;
            _componentContext.SharedData[SharedDataKeys.OptimizeSelfLearningHighVolatilityMultiplier] = _optimizeSelfLearningHighVolatilityMultiplier;
            _componentContext.SharedData[SharedDataKeys.OptimizeSelfLearningLowVolatilityMultiplier] = _optimizeSelfLearningLowVolatilityMultiplier;
            _componentContext.SharedData[SharedDataKeys.OptimizeSelfLearningMinDistance] = _optimizeSelfLearningMinDistance;
            _componentContext.SharedData[SharedDataKeys.OptimizeSelfLearningMaxDistance] = _optimizeSelfLearningMaxDistance;
            _componentContext.SharedData[SharedDataKeys.OptimizeSelfLearningHistorySize] = _optimizeSelfLearningHistorySize;
            _componentContext.SharedData[SharedDataKeys.OptimizeSelfLearningProfitThresholdHigh] = _optimizeSelfLearningProfitThresholdHigh;
            _componentContext.SharedData[SharedDataKeys.OptimizeSelfLearningProfitThresholdLow] = _optimizeSelfLearningProfitThresholdLow;
            _componentContext.SharedData[SharedDataKeys.OptimizeSelfLearningVolatilityThresholdHigh] = _optimizeSelfLearningVolatilityThresholdHigh;
            _componentContext.SharedData[SharedDataKeys.OptimizeSelfLearningVolatilityThresholdLow] = _optimizeSelfLearningVolatilityThresholdLow;
            ...
            // ✅ RSI флаги оптимизации - ДОБАВИТЬ СЮДА:
            _componentContext.SharedData[SharedDataKeys.OptimizeRsiPeriod] = _optimizeRsiPeriod;
            _componentContext.SharedData[SharedDataKeys.OptimizeRsiOversold] = _optimizeRsiOversold;
            _componentContext.SharedData[SharedDataKeys.OptimizeRsiOverbought] = _optimizeRsiOverbought;
            
            // ✅ Параметры логирования
            _componentContext.SharedData["LogVerbosity"] = _logVerbosity;
            _componentContext.SharedData["UseManualTakeProfit"] = _useManualTakeProfit;
            _componentContext.SharedData["ManualTakeProfit"] = _manualTakeProfit;
            _componentContext.SharedData["MinProfitPercent"] = _minProfitPercentParam;
            _componentContext.SharedData["LastAverageBreakevenPriceLogTime"] = _lastAverageBreakevenPriceLogTime;
            _componentContext.SharedData["LastStopStatusLogTime"] = _lastStopStatusLogTime;
        }
        ...
        private async void OnCandleFinishedEvent(List<Candle> candles)
        {
            ...
            // Обработка триггеров логирования из компонента
            if (_componentContext?.SharedData.TryGetValue("TriggerAverageBreakevenPriceLog", out var triggerAvgObj) == true
                && triggerAvgObj is bool triggerAvg && triggerAvg)
            {
                LogAverageBreakevenPrice();
                _componentContext.SharedData["TriggerAverageBreakevenPriceLog"] = false;
            }
            
            if (_componentContext?.SharedData.TryGetValue("TriggerStopStatusLog", out var triggerStopObj) == true
                && triggerStopObj is bool triggerStop && triggerStop)
            {
                decimal logPrice = lastCandle.Close;
                string logVerbosity = "Минимальная";
                if (_componentContext.SharedData.TryGetValue("StopStatusLogPrice", out var priceObj) && priceObj is decimal price)
                {
                    logPrice = price;
                }
                if (_componentContext.SharedData.TryGetValue("StopStatusLogVerbosity", out var verbObj) && verbObj is string verb)
                {
                    logVerbosity = verb;
                }
                LogStopStatus(logPrice, logVerbosity);
                _componentContext.SharedData["TriggerStopStatusLog"] = false;
            }
        }
        
        private async void OnCandleFinishedEventOld(List<Candle> candles)
        {
            try
            {
                ...
                int intervalMinutes = _logVerbosity.ValueString == "Подробная"
                    ? 5
                    : (_logVerbosity.ValueString == "Обычная" ? 15 : 30);
        ...
        private void LogPositionsForced()
        {
            try
            {
                ...
                foreach (var pos in positions)
                {
                    ...
                    // ✅ Получаем сохраненные причины открытия и закрытия
                    string openReason = positionManager.GetOpenReason(id);
                    string openReasonText = string.IsNullOrWhiteSpace(openReason) ? "неизвестен" : openReason;
                    
                    // Для позиций в состоянии Closing показываем причину закрытия, если она сохранена
                    string closeReasonText = "";
                    if (pos.State == PositionStateType.Closing)
                    {
                        string closeReason = positionManager.GetCloseReason(id);
                        if (!string.IsNullOrWhiteSpace(closeReason) && closeReason != "неизвестен")
                        {
                            closeReasonText = $" | закрытие: {closeReason}";
                        }
                    }
                    ...
                    SendNewLogMessage(
                        ...
                        $"целевой выход {expectedExit:F4} ({exitSource}) | " +
                        $"открытие: {openReasonText}{closeReasonText}",
                        LogMessageType.System);
                }
                SendNewLogMessage("=========================================", LogMessageType.System);
            }
            ...
            TrailingStopComponent trailingComponent, decimal entryPrice, decimal currentPrice = 0m)
        {
            ...
            // ✅ ФОЛБЭК: Если minProfit ещё не посчитан в RiskManager, считаем его сами от цены входа
            if (minProfitPrice <= 0 && entryPrice > 0 && _minProfitPercentParam != null)
            {
                decimal minProfitPercent = _minProfitPercentParam.ValueDecimal;
                if (minProfitPercent > 0)
                {
                    minProfitPrice = isLong
                        ? entryPrice * (1 + minProfitPercent / 100m)
                        : entryPrice * (1 - minProfitPercent / 100m);
                }
            }
        ...
        /// <summary>
        /// Логирование средней цены безубытка по всем позициям (раз в 10 минут в режиме "Подробный")
        /// Средняя цена безубытка = средняя цена входа по всем позициям (LONG и SHORT) + минимальная прибыль из параметров
        /// </summary>
        private void LogAverageBreakevenPrice()
        {
            try
            {
                var positionManager = _assembly?.GetComponent<PositionManagerComponent>();
                ...
                if (positionManager == null || riskManager == null)
                    return;
                ...
                decimal minProfitPercent = _minProfitPercentParam?.ValueDecimal ?? 0.14m;
                decimal totalEntryPrice = 0m;
                decimal totalBreakevenPrice = 0m;
                int positionCount = 0;
                int longPositions = 0;
                int shortPositions = 0;
                
                // Рассчитываем цену безубытка для каждой позиции отдельно, затем усредняем
                foreach (var position in openPositions)
                {
                    ...
                    if (entryPrice > 0)
                    {
                        ...
                        // Рассчитываем цену безубытка для данной позиции с учетом направления
                        decimal breakevenPrice;
                        if (position.Direction == Side.Buy)
                        {
                            // Для LONG: цена безубытка = цена входа * (1 + minProfitPercent / 100)
                            breakevenPrice = entryPrice * (1 + minProfitPercent / 100m);
                            longPositions++;
                        }
                        else
                        {
                            // Для SHORT: цена безубытка = цена входа * (1 - minProfitPercent / 100)
                            breakevenPrice = entryPrice * (1 - minProfitPercent / 100m);
                            shortPositions++;
                        }
                        
                        totalBreakevenPrice += breakevenPrice;
                        positionCount++;
                    }
                }
                ...
                // Средняя цена входа по всем позициям
                decimal averageEntryPrice = totalEntryPrice / positionCount;
                
                // Средняя цена безубытка (усредненная по всем позициям с учетом их направления)
                decimal averageBreakevenPrice = totalBreakevenPrice / positionCount;
                
                SendNewLogMessage(
                    $"📊 СРЕДНЯЯ ЦЕНА БЕЗУБЫТКА: {averageBreakevenPrice:F4} " +
                    $"(средняя вход: {averageEntryPrice:F4}, мин.прибыль: {minProfitPercent:F2}%, " +
                    $"позиций: {positionCount}, LONG: {longPositions}, SHORT: {shortPositions})",
                    LogMessageType.System);
            }
            catch (Exception ex)
            {
                SendNewLogMessage($"❌ Ошибка расчета средней цены безубытка: {ex.Message}", LogMessageType.Error);
            }
        }
        
        /// <summary>
        /// Логирование статуса стопов (следящего и фиксированного) для всех режимов
        /// Включает: безубыток, проценты, целевой уровень закрытия
        /// Детальность зависит от режима логирования
        /// </summary>
        private void LogStopStatus(decimal currentPrice, string logVerbosity)
        {
            try
            {
                var positionManager = _assembly?.GetComponent<PositionManagerComponent>();
                ...
                bool isDetailed = logVerbosity == "Подробная";
                bool isNormal = logVerbosity == "Обычная";
                bool isMinimal = logVerbosity == "Минимальная";

                if (isDetailed)
                {
                    SendNewLogMessage("=== 📊 СТАТУС СТОПОВ (СЛЕДЯЩИЙ И ФИКСИРОВАННЫЙ) ===", LogMessageType.System);
                }
                ...
                foreach (var position in openPositions)
                {
                    ...
                    // Получаем минимальную цену прибыли
                    decimal minProfitPrice = riskManager.GetMinProfitPrice(positionId);
                    if (minProfitPrice == 0)
                    {
                        decimal minProfitPercent = _minProfitPercentParam?.ValueDecimal ?? 0.14m;
                    ...
                    // Проверяем безубыток
                    bool breakEvenEnabled = _useBreakEven?.ValueString == "Включён";
                    bool breakEvenApplied = riskManager?.WentPositive(positionId) ?? false;
                    decimal breakEvenLevel = entryPrice; // Безубыток = цена входа
                    decimal breakEvenTriggerPercent = _breakEvenTriggerPercent?.ValueDecimal ?? 0.10m;
                    ...
                    decimal trailingLevel = 0m;
                    string trailingType = "";
                    decimal trailingDistancePercent = 0m;

                    if (trailingEnabled)
                    {
                        trailingActive = trailingComponent.IsTrailingActive(positionId);
                        trailingLevel = trailingComponent.GetTrailingLevel(positionId);
                        trailingType = trailingComponent.GetTrailingType();
                        trailingDistancePercent = trailingComponent.GetTrailingDistancePercent();
                    }

                    // Фиксированный стоп (ManualTakeProfit)
                    bool takeProfitEnabled = _useManualTakeProfit != null && _useManualTakeProfit.ValueString == "Включён";
                    decimal takeProfitLevel = 0m;
                    decimal takeProfitPercent = 0m;

                    if (takeProfitEnabled && _manualTakeProfit != null)
                    {
                        takeProfitPercent = _manualTakeProfit.ValueDecimal;
                        takeProfitLevel = isLong
                            ? entryPrice * (1 + takeProfitPercent / 100m)
                            : entryPrice * (1 - takeProfitPercent / 100m);
                    }

                    // Получаем текущий рассчитанный целевой уровень закрытия
                    ExitInfo exitInfo = GetExpectedExitPrice(position, riskManager, trailingComponent, entryPrice, currentPrice);
                    decimal targetExitLevel = exitInfo.Price;
                    string exitSource = exitInfo.Source;

                    // Формируем лог в зависимости от режима
                    string direction = isLong ? "LONG" : "SHORT";

                    if (isDetailed)
                    {
                        // Подробный режим - полная информация
                        SendNewLogMessage($"🔹 ПОЗИЦИЯ #{positionId} ({direction})", LogMessageType.System);
                        SendNewLogMessage($"   💰 ТЕКУЩАЯ ПРИБЫЛЬ: {currentProfitPercent:F2}% (цена: {currentPrice:F4}, вход: {entryPrice:F4})", LogMessageType.System);
                        SendNewLogMessage($"   🎯 МИН. ПРИБЫЛЬ: {minProfitPrice:F4} (порог: {_minProfitPercentParam?.ValueDecimal ?? 0.14m:F2}%)", LogMessageType.System);

                        // Безубыток
                        if (breakEvenEnabled)
                        {
                            string beStatus = breakEvenApplied ? "✅ ПРИМЕНЕН" : $"⏳ ОЖИДАНИЕ (триггер: {breakEvenTriggerPercent:F2}%)";
                            SendNewLogMessage($"   ⚖️ БЕЗУБЫТОК: {beStatus} | Уровень: {breakEvenLevel:F4}", LogMessageType.System);
                        }
                        else
                        {
                            SendNewLogMessage($"   ⚖️ БЕЗУБЫТОК: ❌ ВЫКЛЮЧЕН", LogMessageType.System);
                        }
                        ...
                        if (trailingEnabled)
                        {
                            if (trailingActive && trailingLevel > 0)
                            {
                                ...
                                string trailingStatus = isLong
                                    ? (currentPrice > trailingLevel ? "✅ АКТИВЕН" : "🔔 СРАБОТАЛ")
                                    : (currentPrice < trailingLevel ? "✅ АКТИВЕН" : "🔔 СРАБОТАЛ");

                                SendNewLogMessage($"   📌 СЛЕДЯЩИЙ СТОП: {trailingStatus} | Тип: {trailingType} | Уровень: {trailingLevel:F4} | Расстояние: {distancePercent:F3}% ({distanceToTrailing:F4})", LogMessageType.System);
                            }
                            else
                            {
                                SendNewLogMessage($"   📌 СЛЕДЯЩИЙ СТОП: ⏳ НЕ АКТИВЕН (старт при {_trailingStartPercent?.ValueDecimal ?? 0m:F2}% прибыли)", LogMessageType.System);
                            }
                        }
                        else
                        {
                            SendNewLogMessage($"   📌 СЛЕДЯЩИЙ СТОП: ❌ ВЫКЛЮЧЕН", LogMessageType.System);
                        }

                        // Фиксированный стоп (ManualTakeProfit)
                        if (takeProfitEnabled)
                        {
                            decimal distanceToTP = isLong
                                ? (takeProfitLevel - currentPrice)
                                : (currentPrice - takeProfitLevel);
                            decimal distanceTPPercent = (distanceToTP / entryPrice) * 100m;

                            string tpStatus = isLong
                                ? (currentPrice >= takeProfitLevel ? "🔔 ДОСТИГНУТ" : "⏳ ОЖИДАНИЕ")
                                : (currentPrice <= takeProfitLevel ? "🔔 ДОСТИГНУТ" : "⏳ ОЖИДАНИЕ");

                            SendNewLogMessage($"   🎯 ФИКСИРОВАННЫЙ TP: {tpStatus} | Уровень: {takeProfitLevel:F4} ({takeProfitPercent:F2}%) | Расстояние: {distanceTPPercent:F3}% ({distanceToTP:F4})", LogMessageType.System);
                        }
                        else
                        {
                            SendNewLogMessage($"   🎯 ФИКСИРОВАННЫЙ TP: ❌ ВЫКЛЮЧЕН", LogMessageType.System);
                        }

                        // Текущий рассчитанный целевой уровень закрытия
                        SendNewLogMessage($"   🎯 ЦЕЛЕВОЙ УРОВЕНЬ ЗАКРЫТИЯ: {targetExitLevel:F4} (источник: {exitSource})", LogMessageType.System);

                        SendNewLogMessage("", LogMessageType.System); // Пустая строка для разделения
                    }
                    else if (isNormal)
                    {
                        // Обычный режим - краткая информация
                        string stopInfo = "";

                        if (trailingEnabled && trailingActive && trailingLevel > 0)
                        {
                            string trailingStatus = isLong
                                ? (currentPrice > trailingLevel ? "✅" : "🔔")
                                : (currentPrice < trailingLevel ? "✅" : "🔔");
                            stopInfo += $" Трейлинг: {trailingStatus} {trailingLevel:F4}";
                        }
                        ...
                        if (takeProfitEnabled)
                        {
                            string tpStatus = isLong
                                ? (currentPrice >= takeProfitLevel ? "🔔" : "⏳")
                                : (currentPrice <= takeProfitLevel ? "🔔" : "⏳");
                            stopInfo += $" TP: {tpStatus} {takeProfitLevel:F4}";
                        }

                        if (!string.IsNullOrEmpty(stopInfo))
                        {
                            SendNewLogMessage($"📊 Позиция #{positionId} ({direction}): Прибыль {currentProfitPercent:F2}% |{stopInfo} | Цель: {targetExitLevel:F4}", LogMessageType.System);
                        }
                    }
                    else if (isMinimal)
                    {
                        // Минимальный режим - только критическая информация
                        if (trailingEnabled && trailingActive && trailingLevel > 0)
                        {
                            bool stopHit = isLong ? currentPrice <= trailingLevel : currentPrice >= trailingLevel;
                            if (stopHit)
                            {
                                SendNewLogMessage($"🔔 Трейлинг-стоп сработал для позиции #{positionId} ({direction})", LogMessageType.System);
                            }
                        }

                        if (takeProfitEnabled)
                        {
                            bool tpHit = isLong ? currentPrice >= takeProfitLevel : currentPrice <= takeProfitLevel;
                            if (tpHit)
                            {
                                SendNewLogMessage($"🔔 Тейк-профит достигнут для позиции #{positionId} ({direction})", LogMessageType.System);
                            }
                        }
                    }
                }
            ...
            {
                SendNewLogMessage($"❌ Ошибка логирования статуса стопов: {ex.Message}", LogMessageType.Error);
            }
        }

        /// <summary>
        /// Проверка фильтра объема: сравнивает текущий объем с средним объемом за период
        /// </summary>
        private bool IsVolumeFilterPassed(Candle candle)
        {
            ...
            {
                var tab = _context.GetTab();
                if (tab?.CandlesAll == null || _volumePeriod == null || _volumeMultiplier == null)
                    return true;

                int volumePeriod = _volumePeriod.ValueInt;
                decimal volumeMultiplier = _volumeMultiplier.ValueDecimal;

                if (volumePeriod <= 0 || volumeMultiplier <= 0)
                    return true;

                // Получаем свечи для расчета среднего объема
                var candles = tab.CandlesAll;
                int count = candles.Count;

                if (count < volumePeriod + 1)
                    return true; // Недостаточно данных для расчета

                // Рассчитываем средний объем за период (без текущей свечи)
                decimal totalVolume = 0m;
                int candlesToAverage = Math.Min(volumePeriod, count - 1);

                for (int i = count - candlesToAverage - 1; i < count - 1; i++)
                {
                    if (i >= 0)
                    {
                ...
                decimal averageVolume = totalVolume / candlesToAverage;

                // Проверяем, что текущий объем >= средний объем * множитель
                decimal requiredVolume = averageVolume * volumeMultiplier;
                bool volumeFilterPassed = candle.Volume >= requiredVolume;

                if (!volumeFilterPassed)
                {
                    _context.SendLog($"🚫 ФИЛЬТР ОБЪЕМА: текущий объем {candle.Volume:F0} < требуемый {requiredVolume:F0} (средний {averageVolume:F0} × {volumeMultiplier:F1})",
                        LogMessageType.System);
                }

                return volumeFilterPassed;
            }
            catch (Exception ex)
            {
                _context.SendLog($"⚠️ Ошибка проверки фильтра объема: {ex.Message}", LogMessageType.Error);
                return true; // В случае ошибки разрешаем торговлю
            }
        }
